 ...
 * a spec gap. The median reads through the same `.lu-stat-*` classes the
 ...
 * actually debuted for a median to mean anything. A DJ with 500 qualifying
...
export function TimeToFirstPlay({ model }: { model: TimeToFirstPlayModel }) {
  ...
  const showMedian = hasEnoughTimeToFirstPlayDebuts(model);
  ...
  return (
    <section className="lu-module dz-shell" aria-label={summary}>
      ...
      {!hasEnoughTimeToFirstPlayTracks(model) ? (
        ...
        <>
          {/* Scoped to the tracks that actually debuted, matching the
              `aria-label`. The first shipped version read a bare "median from
              add to first spin", so a sighted user was told the median covered
              the whole library while the accessible name correctly scoped
              it — with 59% of the population never played on real data
              (Story 4.5 review). */}
          <p className="lu-stat-readout" aria-hidden="true">
            <span className="lu-stat-value">{formatElapsed(model.medianElapsedMs as number)}</span> median from add to
            first spin, across the {playedCount} tracks that have debuted
          </p>
          {model.neverPlayedCount > 0 && (
            ...
            <p className="lu-disclosure" aria-hidden="true">
              {model.neverPlayedCount} {model.neverPlayedCount === 1 ? "track hasn't" : "tracks haven't"} been played
              yet
              {model.neverPlayedMedianAgeMs !== null &&
                ` — a median of ${formatElapsed(model.neverPlayedMedianAgeMs)} on the shelf`}
            </p>
          )}
        </>
      ) : (
