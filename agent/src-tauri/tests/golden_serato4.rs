...
/// sentinel + empty-string "absent" convention (row 2) and the multi-deck range "1"-"4"
/// real data confirms occurs.
#[test]
fn golden_session_reads_and_joins_match_expected_output() {
    ...
    assert_eq!(
        ...
        "row 2's empty-string genre/key resolve to None, not Some(\"\"); end_time=-1 does not affect this read"
    );
