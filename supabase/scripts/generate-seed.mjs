...
// Story 5.2: dancefloor segments per set, GENERATED BY THE RUST DETECTOR from
// these same fixture sets (`agent/src-tauri/tests/segments_fixture.rs`), never
// hand-authored here. Reimplementing detection in this script would be a second
// algorithm, which is precisely what D-1 forbids. That test also fails if this
// file drifts from the algorithm, so a stale seed cannot go unnoticed:
//   CURFEW_WRITE_SEGMENTS_FIXTURE=1 cargo test --manifest-path agent/src-tauri/Cargo.toml --test segments_fixture
const segments = JSON.parse(readFileSync(join(fixtures, "segments.fixture.json"), "utf-8"));
...
const segmentCount = segments.reduce((a, s) => a + (s.segments?.length ?? 0), 0);
const setsNoSegment = segments.filter((s) => (s.segments?.length ?? 0) === 0).length;
const setsManySegments = segments.filter((s) => (s.segments?.length ?? 0) > 1).length;
w(
  `--   ${segmentCount} dancefloor segments (Story 5.2): ${setsNoSegment} sets with none,`,
);
w(
  `--   ${setsManySegments} with several — the zero/one/several states FR-28 requires and the`,
);
w("--   whole-set fallback the card/hero take when a set has no segment at all");
...
w(`delete from public.segments where dj_id = ${q(DJ_ID)};`);
...
// Dancefloor segments (Story 5.2). These MUST resolve to the `plays.id`s this
// seed just inserted -- and those ids are `gen_random_uuid()` defaults, so there
// is nothing literal to point at. The real write path has `sync_set` resolve
// position -> uuid inside its own transaction; a seed has no RPC to do that, so
// it does the same resolution in SQL, joining each (set, position) pair to the
// plays row that now holds it. A segment whose boundary position is missing
// simply produces no row (inner join), which mirrors the RPC skipping an entry
// it cannot resolve rather than failing the whole load.
w("-- Dancefloor segments (Story 5.2, D-24) — what the agent's detector produced");
w("-- for these exact sets, so the local browser pass exercises the real read");
w("-- path (`segments` rows), not a client-side recomputation. Boundary");
w("-- positions are resolved to the plays rows above the same way sync_set does");
w("-- it, since seed.sql inserts plays directly and has no RPC to resolve them.");
const segmentTuples = [];
for (const entry of segments) {
  const setId = uuidFrom(`set:${entry.external_id}`);
  for (const s of entry.segments ?? []) {
    segmentTuples.push(`  (${q(setId)}::uuid, ${q(s.first_position)}, ${q(s.last_position)})`);
  }
}
for (let i = 0; i < segmentTuples.length; i += CHUNK) {
  w(
    "insert into public.segments (set_id, dj_id, type, first_play_id, last_play_id, source, confirmed)",
  );
  w(`select v.set_id, ${q(DJ_ID)}, 'dancefloor', fp.id, lp.id, 'suggested', false`);
  w("from (values");
  w(`${segmentTuples.slice(i, i + CHUNK).join(",\n")}`);
  w(") as v(set_id, first_position, last_position)");
  w("join public.plays fp on fp.set_id = v.set_id and fp.position = v.first_position");
  w("join public.plays lp on lp.set_id = v.set_id and lp.position = v.last_position;");
  w("");
}
