...
| Repeat-track rate | Library Utilization | *(Story 4.9, implemented 2026-08-08, in review — no prior wireframe existed for this component.)* A single-figure stat tile reusing the `.lu-stat-*` typography, paired in one row with Rotation size under a "Rotation" heading. States the percentage **and the number of nights it averaged over**, because a mean over one night and a mean over forty are different claims that the percentage alone cannot distinguish. The definition is fixed in epics.md Story 4.9 AC-3 and must not drift: the unweighted mean of per-set carryover against each set's own up-to-5 **predecessors**, oldest set excluded rather than scored 0%. Below the gate it states no figure — and its accessible name states none either. |
| Set similarity | Library Utilization | *(Story 4.9, implemented 2026-08-08, in review — the only page whose Stitch wireframe covers this component, and the wireframe's treatment is deliberately **rejected**; see below.)* A pairwise-overlap matrix across the **10 most recent** qualifying sets, and it **states that cap on screen** whenever more sets exist than it shows — a silent top-10 truncation reads as "this is all your history". Intensity rides opacity over a token fill, but **every populated cell also prints its own percentage as text**, so intensity is never carried by colour alone (Accessibility Floor; WCAG SC 1.4.1). The grid is hidden from assistive tech and the module's accessible name carries the text equivalent instead — naming the most-alike pair and its share in words — because a 10×10 table announced cell by cell is 100 announcements to reach one fact. **Degrades to a ranked "most alike" list at phone widths**, via CSS alone: a 10-column grid cannot give every cell 24×24 inside the page gutters at 320px, so the list is the phone experience rather than an edge case (same pattern as Story 4.8's Camelot wheel). The mock's `overflow-x: auto` over a 600px-min-width grid is rejected outright — a horizontally-scrolling region at 320px is exactly the overflow the accessibility AC forbids, and it leaves the cells under 24×24 anyway. Line 31's spine-over-mocks rule governs, and the mock predates the Ice Cyan / Abyss redesign besides. |
| Workhorses | Library Utilization | *(Story 4.9, implemented 2026-08-08, in review — no prior wireframe existed for this component.)* A track-row list ranked by the **number of sets a track appeared in**, paired with Played-once under a "Tracks" heading. **Deliberately a different question from the dashboard's most-played card** (play count, last 10 sets, dancefloor-segment-scoped) and the two are expected to disagree — a future reader will be tempted to reconcile them and must not. Play count is shown beside the set count as context, never as the rank. Rows cap at a named constant with a "load more" disclosure — infinite scroll on track lists is banned (Interaction Primitives) — and the disclosure is a native `<details>`, which keeps the module a server component and gives it a keyboard path for free. **No ranking vocabulary anywhere**: no "top", no "best", no "#1", no medals, no badges, and no row numbers (DESIGN.md's total-absence-of-competitive-cues rule). A missing artist renders "Unknown", never blank. |
| Played once (one-and-done) | Library Utilization | *(Story 4.9, implemented 2026-08-08, in review — no prior wireframe existed for this component.)* Same row-list pattern as Workhorses, and its deliberate mirror: tracks played **exactly once** across the qualifying population, by total plays — a track played twice inside a single set does not qualify, because the DJ came back to it. Ordered **most-recently-played first**, because the list exists to be acted on and the track tried last week is the one worth another look; alphabetical would be a filing system rather than a prompt. Carries the date of that single play, since "once, in March" and "once, on Friday" are different prompts; an unparseable play time renders an em dash rather than a guessed date. **Reads as a complement to the Aging-shelf row above, not a duplicate:** this list is about tracks the DJ *did* play and dropped, the shelf is about tracks never reached at all. |
| Rotation size | Library Utilization | *(Story 4.9, implemented 2026-08-08, in review — no prior wireframe existed for this component.)* A two-figure stat tile rendering as "N plays, M unique" over a **fixed trailing 60-day window**, and **naming that window in its own copy**. Fixed rather than following the conversion pair's selectable dropdown: a figure the dropdown cannot move must not sit beneath it, and a fixed window that does not say so is indistinguishable on screen from a broken one. For the same reason it renders **outside** the conversion section entirely — nesting a play-side stat there would file it under the `<h2>Conversion</h2>` heading, which is not what it measures. (This row cited an `aria-label="Conversion"` landmark until 2026-08-08; the same story that added the row removed that label as part of the heading-outline fix, so the justification was false on arrival.) |
...
| Track search | Library Utilization | *(Story 4.10, implemented 2026-08-10 — no prior wireframe existed for this component.)* A single live-filtering field at the head of the page, reusing the dashboard Spotlight pill's visual language and its accessibility shape (real `<input>` with an explicit `aria-label`, decorative placeholder `aria-hidden`, icons `aria-hidden`) but **none of its mechanics** — no sort chips, no gooey blob filter, no `framer-motion`; the results' entrance is CSS inside a reduced-motion guard. Filters **client-side** over a compact index the server builds from the two arrays the page already loads: there is no index on `plays.title`/`artist` or `library_roster.title`/`artist` and no trigram/full-text extension anywhere, so a server-side `ilike` would be a sequential scan and a GIN index would be a migration. Every whitespace-split token must hit the haystack, matching the set-list panel's filter exactly. **Searches played ∪ owned, and each result states which it is** — played rows carry their play and set counts, owned rows read "Not played yet · added ⟨date⟩" (or "add date unknown", never a guessed one). Rows whose track has no resolvable identity render as plain text rather than dead links, and the page states how many in one sentence. Results cap at a named constant with the **full match count and which end is shown** stated alongside; a native `<details>` carries the overflow, since infinite scroll on track lists is banned. The query is unpersisted `useState` — a window persists, a transient view does not — and no results render at all until the DJ types, because a results list is a response to a question. **It sits above the page's one low-confidence reveal and is governed by it**, so search counts and every other figure on the page always describe the same set population; an earlier build gave it its own reveal and put two identical "N sessions hidden" controls on screen 200px apart. |
| Track detail | `/track/[track_id]` | *(Story 4.10, implemented 2026-08-10 — no prior wireframe existed for this screen.)* Whole-page scroll on the same Silk ground as Set Detail, so the two detail screens read as one pattern. Header is the track title as `<h1>`, artist beneath, then a tag row — BPM, key, genre → subgenre, add date — where **every absent field is named "Unknown"**, never blank and never guessed, and the add date is never defaulted to the first play. Below it, four modules in the page's standard glass shell: **play history** (times played, first and last, and the distinct sets as rows linking into `/set/[id]`, labelled by session id and never by the raw uuid), a **clock-time strip**, **ride time**, and **mix neighbours**. The clock strip's hour labels and its text equivalent are both computed **in the browser, in the viewer's own timezone** — nothing in the system stores a venue, DJ or set offset, so a server-rendered hour would be wrong for every DJ not sitting in the server's zone and a hydration mismatch besides; before hydration the module shows an honest placeholder rather than a strip of zeroes. Ride time is a **median with its n stated**, and at n=1 it renders the single duration and never says "typically". Mix neighbours are `position ± 1` **within one set only** and ordered by recurrence, with no ranking vocabulary and no row numbers; a track that follows itself is a real answer. The whole body honours the same exclude-visibly reveal as `/library-utilization`. **A track the DJ owns but has never played is a designed state, not an error branch** — identity, add date and one honest line, rather than four empty modules — and it is the first surface in the product that says something true on day one. An unknown id 404s exactly like `/set/[id]`, with no message distinguishing "doesn't exist" from "not yours"; RLS makes them indistinguishable by design and that is the correct privacy posture. |
