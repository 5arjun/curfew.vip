...
|---|---|---|
| Auth form (Login/Signup) | Login / Signup, overlaid on Landing | Four paths, DJ's choice (FR-29): manual name, email, phone, and password fields, Google, Apple, or passkey (WebAuthn). All four resolve to one account, auto-linked by verified email — no account picker, no duplicate-account risk if a DJ mixes methods across devices. Google/Apple sign-ups pull name + email from the provider and skip those manual fields, but still hit the phone-number step (see State Patterns) before the account is usable. |
