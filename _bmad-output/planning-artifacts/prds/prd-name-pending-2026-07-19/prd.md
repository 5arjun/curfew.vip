...
> **Decision A — launch ingestion is go-forward only (Arjun, 2026-07-25).** At launch, the agent captures and analyzes only sessions completed *after* the DJ subscribes — no bulk-import of an existing `History/Sessions/` folder or `master.sqlite` history, no historical backfill. The library metadata DB is still read live (BPM/key/genre enrichment, add-dates); the product is blind only to "was this track played before we were watching." Consequences: the empty/sparse dashboard **is** the launch experience — every Phase 1 feature must read honestly against zero-to-thin history, and copy matures as data accrues rather than assuming a "six-months-in" DJ from day one. This is also the retention moat: cancelling leaves a permanent, unfillable gap in a DJ's own evolution timeline. See `epics.md` Overview and Architecture Spine AD-4 for the propagated framing; full rationale in `implementation-artifacts/epic-1-review-decisions-2026-07-25.md`.
...
- **Played** — Throughout this PRD and its features, "played" means *played in a Set Curfew captured* — i.e., go-forward from the DJ's subscription date, never lifetime Serato history (Decision A, §1 Vision). A track a DJ has spun for years but never played since subscribing reads as unplayed everywhere in the product (Style Evolution, conversion rate, aging shelf, time-to-first-play). Copy must never frame this as a receipt of elapsed subscription time (no "since you joined"); always frame it as the DJ's own record, earning out as history accrues.
...
**Description:** Surfaces whether a DJ's library spending translates into actual playing, and how they are *using* what they already own — whether recently-added tracks make it into sets, conversion rate, aging shelf, time-to-first-play, the play-side utilization stats (repeat-track rate, set similarity, workhorses, one-and-done, rotation size), and **look-up of any single track with its own detail view** — all derived from library and session data with no manual input. *(The four-item list this Description carried until 2026-08-08 predated FR-30 and was left incomplete when those five metrics landed — the same accretion FR-9's sectioned composition and FR-10's §4.3→§4.4 move each had to close. Amended with FR-30 rather than after it, and with FR-31 on 2026-08-10 for the same reason: every other item here is an **aggregate**, and FR-31 is the section's only single-record view, which is exactly the kind of addition a Description silently outgrows.)*
...
#### FR-31: Track lookup and track detail

DJ can look up any single track — by title or artist, across both the tracks they have played and the tracks in their synced library — and open a detail view for it showing its tags, how often and when they have played it, which sets it appeared in, what time of night it lands, how long they ride it, and what they mix it with. *(Added 2026-08-10 alongside epics.md Story 4.10, the last story in Epic 4. A post-PRD extension of the same shape FR-30 and FR-9's sectioned composition had to close after the fact. **This is §4.4's only single-record view** — everything else in the section is an aggregate — which is why it needs its own FR rather than a consequence bullet under one.)*

**Consequences (testable):**
- **Search covers played ∪ owned, and every result says which it is.** A track with no plays has two completely different causes — never reached yet, versus played but untagged — and the DJ must never have to guess which they are looking at. The field's label names both populations: "your library" alone over-promises on the ~28% of catalogue rows with no resolvable identity, and "tracks you've played" alone stopped being true when FR-12's roster landed. *(Ruled 2026-08-10, Arjun; epics.md Story 4.10 D-25, discharging Story 4.11 AC-9.)*
- **A track with no resolvable identity is readable but not openable, and the count is disclosed.** The detail route is keyed on `track_id`, which requires **both** a title and an artist tag (AD-11) — so ~21% of real plays, and 212 of the committed seed's 1,267 distinct tracks, have no page. Those rows still appear and are still readable; they render unlinked rather than as dead links, and one sentence on the page states how many. *(Story 4.10 D-26; SM-C1.)*
- **Clock-time labels are produced in the viewer's own timezone, never server-rendered.** `plays.started_at` is stored as UTC with the capture-side offset lost, and no venue, DJ or set timezone is stored anywhere — so a server-rendered hour is both wrong and a hydration mismatch. The viewer's zone is also the only correct one: the DJ plays where they live. *(Story 4.10 D-32.)*
- **Ride time is a median with its n stated**, and at n=1 it renders the single duration and never the word "typically" — one observation is a fact about one night, not a distribution. Plays missing a duration are excluded **with their count disclosed**, and that count survives the case where every play is missing one. *(Story 4.10 D-33; SM-C1.)*
- **Mix neighbours are ordered by recurrence and never cross a set boundary** — the last track of one night is not the neighbour of the first track of the next. Ordered, but described in no ranking vocabulary. *(Story 4.10 D-31; DESIGN.md's no-competitive-cues rule.)*
- **Both surfaces honour the same page-wide exclude-visibly contract** as the rest of §4.4, under the one reveal that governs every figure on the screen (FR-27) — a track's play count must not be silently inflated by soundchecks, nor silently reduced without the DJ being told.
- **A DJ with a synced roster and zero sets gets a working search and a real detail page** for the tracks they own — identity, add date, and an honest "not played yet" rather than four empty modules. This is the first surface in the product that says something true on day one. *(Story 4.10 D-38.)*
...
1. ~~Set-boundary detection (where a set starts/ends) is unvalidated against real multi-track gig data — blocking gate for FR-1/FR-27/FR-28 (brief's O-4).~~ **RESOLVED (2026-07-20)** — validated against a real 474-session corpus: a density + DJ-relative-BPM-floor heuristic, confirmed by transition-smoothness, correctly handled dancefloor-throughout club sets and a bundled multi-block session. Closes SM-1/brief O-4. See Architecture Spine AD-17 / Open Questions #1.
...
3. ~~Does Serato's library DB reliably expose a "date added to library" field? Needed for FR-11–FR-13; unconfirmed by domain research.~~ **RESOLVED (2026-07-20)** — inspection of a real `database V2` (929 tracks) found `tadd` present at ~94% coverage (plus a `uadd` timestamp form). FR-11–FR-13 are buildable; the ~6% missing cases need a graceful "Unknown"-convention fallback (tracked as an epics.md gap). See Architecture Spine Open Questions #2.
4. ~~Formal GDPR/CCPA-equivalent privacy review not yet conducted — advised before public launch (technical research).~~ **PARTIALLY RESOLVED (2026-07-20)** — launch geography decided: US-only at launch, so a CCPA-level posture is sufficient at v1; a GDPR-equivalent review is deferred until international expansion is real. The formal CCPA-compliance review itself remains an outstanding pre-launch checklist item, not an open question. See Architecture Spine Open Questions #6.
