...
- **Rule:** web-side mutations (enrichment, hide, visibility, follow) go through Supabase (PostgREST / `supabase-js`), guarded by RLS. No custom mutation server at v1. The agent's **only** write is the idempotent set sync (AD-4) — except **four** named amendments to this rule: the AD-20 status heartbeat, the AD-21 library add-event batch, the AD-22 library roster batch, and the AD-23 suggested-segment materialization. Any further agent write requires the same treatment (a named AD entry, a `SECURITY DEFINER` RPC, an explicit table/column/row scope, no DJ write grant); an unnamed fifth write is a violation of this rule, not a precedent set by the first four.
  - **AD-23 deliberately breaks the "its own RPC, its own table" half of that template**, and is the reason this clause now says "the same treatment" rather than naming those two specifics. It widens the *existing* `sync_set` onto the *existing* `segments` table, because a suggested segment foreign-keys the very `plays` rows `sync_set` deletes and reinserts — a separate RPC would race its own prerequisite. The parts of the template that carry the actual safety (a named AD, a `SECURITY DEFINER` function, an explicit and narrow write scope, no DJ write grant) all still hold. See AD-23 for the full reasoning; the point of writing it down is that the deviation was decided, not drifted into. *(Amended 2026-08-10, Story 5.2.)*
...
- **Rule:**
  ...
  - The agent's sync upsert is **column-scoped to content columns**. User-authored **overlay** columns — visibility tier, per-track hide, Layer 2 enrichment — are **disjoint and never written by the agent** (the mechanical enforcement of AD-6, contract-tested in `shared/`). An idempotent re-sync can therefore never reset a tier (AD-9); it can never wipe an overlay **except for the one qualified, scoped case the next two bullets carve out**.
  - **Segments are the one qualified case, and the qualification is by ROW, not by column** *(amended 2026-08-10, Story 5.2 — this bullet previously listed "segments" flatly among the disjoint overlay columns, which AD-23 makes false)*: `segments` rows where `source = 'suggested' and not confirmed` are agent-derived content, written and replaced by `sync_set` on every re-sync (AD-23). Rows a DJ authored — `confirmed = true`, or `source = 'manual'` — remain pure overlay and are never written by any sync path. Segment *detection* is content; segment *confirmation and editing* is overlay. The `source`/`confirmed` column pair exists precisely so that line is representable in the schema rather than only in prose.
  - **~~Known violation~~ CLOSED, 2026-08-11, Story 5.3 (AD-24)** *(raised 2026-08-10, Story 5.2, D-21)*: `sync_set` deletes and reinserts a set's `plays` on every call (it has since Story 3.2), and `segments.first_play_id`/`last_play_id` carry `on delete cascade` — so a re-sync removed **every** segments row for that set, DJ-authored ones included. Harmless while only recomputable suggestions existed; real data loss from the moment Story 5.3 wrote the first `confirmed = true` row. **Fixed by capture-and-reinsert** (AD-24): `sync_set` now captures each `confirmed`/`manual` row whole *before* the plays delete and writes it back after, under its original `id` and `created_at`, with each boundary independently re-resolved to the play at its former `position` — clamping to the nearest remaining position with a server-side warning rather than ever deleting the row. The guarantee above is therefore now true for `segments` as it already was for every other overlay, with one asserted exception: a set re-synced with zero plays has no timeline left to bind a boundary to. Proven by `sync_set_isolation_test.sql`'s D-27 cases.
...
- **Refinement, 2026-08-10 (Story 5.2 — implemented, `agent/src-tauri/src/stats/segments.rs`).** Two things this rule described in one shape shipped in another, both within its intent:
  - **Smoothness metric.** "The fraction of consecutive track-pairs with a small BPM delta" needs a second constant (how small is "small") before it can be computed, and that constant would itself be a hardcoded global — the exact thing this AD forbids for the other two signals. It shipped instead as a **window-level median absolute consecutive-pair BPM delta**, with a run confirmed on the median of its windows' values. Same signal, one fewer arbitrary constant, and deliberately outlier-tolerant so a single off-tempo request drop cannot fail an otherwise-tight window. The corpus observation this AD records (~42% smooth for a non-mixed ceremony stretch vs ~65–78% for confirmed mixed sets) is what brackets the metric's cold-start prior; it remains a **confirming gate**, never a candidacy signal.
  - **How "calibrated per-DJ" is computed.** Floors are the **60th percentile** of that DJ's own historical per-window stats, blended toward a shared prior while history is thin (`w = n/(n+5)`) so session one behaves sensibly with no cold-start cliff and no empty state. Percentile rather than mean/σ: robust to one outlier night, and debuggable ("your 60th-percentile window had 4 plays"). The prior is the retired web v0 heuristic's constants — the interim global detector becomes the starting point every DJ's own numbers move away from, never a second system running in parallel.
  - Also settled: the pool is **chronological** (a session calibrates against what preceded it, so re-derivation is byte-stable and does not silently rewrite suggestions a DJ has already seen), and detection emits **`dancefloor` only** — `dinner`/`performance`/`custom` are human labels, and idle stretches are descriptive markers, not rows (there is deliberately no `idle` value in the type enum).
...
### AD-23 — Suggested segments ride the existing set payload; `sync_set` materializes them, row-scoped `[ADOPTED 2026-08-10]`

- **Binds:** FR-28, AR-13; Epic 5 Story 5.2; the agent write path; the `segments` table; AD-8 (explicitly amended, for the fourth time after AD-20/21/22, and the first time by *widening an existing* write rather than adding a new one); AD-16 (whose "segments are never written by the agent" clause this qualifies); AD-17 (the algorithm whose output this transports); AD-3/AD-15 (the additive `shared/` change this rests on).
- **Prevents:** AD-17's detector having **no way to reach the table its output belongs in** — detection runs agent-side in the Rust stat engine (AR-13/epics.md say so explicitly), `segments` is cloud-side, and every other route from one to the other was either forbidden (AD-16's overlay disjointness), racy, or a duplicate algorithm; *and* the opposite failure, a segment write growing ad hoc outside a named boundary the way AD-20 was written to stop the first time.
- **Rule:** AD-8's "the agent's only write is the idempotent set sync" is amended to admit a **fourth** case, and it is a different *shape* from AD-20/21/22 on purpose. Detection output rides the **existing per-set payload** as two additive optional `SyncSetDerived` fields (`suggested_segments`, `idle_gaps`), and the **existing** `sync_set` RPC materializes the rows inside the same transaction that writes the plays. No new RPC, no new queue item, no new table, no new grant.
  - **Write scope, exhaustively:** `sync_set` may insert and delete `segments` rows where `source = 'suggested' and not confirmed`, for the set it is syncing, and nothing else. Rows a DJ authored (`confirmed = true`, or `source = 'manual'`) are untouchable by any sync path — that is the line AD-16 now draws by row rather than by column, and the `source`/`confirmed` column pair exists to make it representable.
  - **No DJ write grant is introduced.** `segments` keeps Story 5.1's zero-write-grant state; the `SECURITY DEFINER` RPC is the only writer, and it derives `set_id`/`dj_id` from `auth.uid()` and its own computed session id, never from the payload. Same RLS-safety property as AD-20/21/22.
- **Why the existing RPC rather than its own (the deliberate deviation from AD-8's template).** A suggested segment foreign-keys the `plays` rows that `sync_set` deletes and reinserts on every call. A separate RPC plus queue item would (a) race its own prerequisite — the rows it points at can be re-minted between the two calls — and (b) need its own position→uuid resolution window anyway. Widening the existing transaction removes the ordering problem by construction. A sixth `sync_set` parameter was also rejected: it would leave a live 5-arg overload to maintain for older agents, for no gain over a strict subset of the `derived` channel the payload already carries. **The safety properties of AD-8's template are all preserved** (named AD, `SECURITY DEFINER`, narrow explicit scope, no DJ write grant); only "its own RPC, its own table" is traded away, knowingly.
- **Why this is content, not overlay.** Suggestions are *derived agent computation over content* — exactly what `derived` is for, and the same category as the energy arc or the genre breakdown. The overlay property attaches to `confirmed`/`manual` **state**, which stays web-authored (Story 5.3). AD-21's own doc-comment test for what belongs on the per-set payload — "is it set-scoped? does re-send-on-re-sync make sense?" — answers **yes** on both counts here, the inverse of the add-event case that had to become its own payload.
- **Wire shape: 1-based positions, RPC-resolved, skip-never-fail.** The agent cannot know a cloud `plays.id` (they are minted, and re-minted, inside `sync_set`), so a suggestion addresses the same payload's `plays[]` by position and the RPC resolves both ends after its own insert. An entry that does not validate (out of range, inverted, non-integer, unsupported type) is **warned about and skipped** — never raised. An overlay nicety must never poison a content sync or wedge the agent's at-least-once retry queue; Epic 5's own charter is that nothing in it is ever required for core dashboard stats.
- **`sets.derived` keeps an inert copy — the web must never read it.** The suggestion blob stays in `derived` as provenance, and after Story 5.3 edits rows it *will* diverge from the `segments` table. The **rows are the sole read model**, guarded by doc comments in `shared/src/index.ts` and at the web read path. Reading the blob would render a suggestion the DJ has already overruled.
- **~~Known hazard, assigned forward~~ CLOSED, 2026-08-11, Story 5.3 (AD-24):** the plays delete+reinsert cascaded through `segments`' FKs and wiped *all* segment rows for a set, DJ-authored ones included. `sync_set` now captures those rows whole before the delete and reinserts them afterwards under their original ids, rebinding each boundary by `position` (clamp-and-warn, never delete). See AD-16's amended bullet and AD-24 for the mechanism, and note that AD-24 is a **DJ-direct write under AD-8's generic clause, not a fifth agent-write amendment** alongside this one.
- **No DJ write grant is introduced *by this AD* — and that sentence is now historical.** Story 5.1's zero-write-grant state held through Story 5.2 and ended in Story 5.3, which granted the DJ scoped INSERT/UPDATE/DELETE under AD-24. The claim above remains true of *this* AD's own write path (`sync_set` is still `SECURITY DEFINER` and still derives `set_id`/`dj_id` itself); it is no longer true of the table.
- **Contract impact:** additive only (AD-15). Two optional `SyncSetDerived` fields; the `PUT /sets/:set_id` idempotency contract, `sync_library_add_events` and `sync_library_roster` are all untouched. `idle_gaps` carries **Unix epoch seconds**, matching what `derived_json` actually crosses this seam as (see the Dates row) rather than the ISO the older field docs aspire to.
- **Status:** `[ADOPTED]` — Story 5.2, 2026-08-10. Recorded here rather than left implicit in code, per the ai-6 failure mode AD-21 names; the AD-8/AD-16/AD-17 amendments above landed in the same pass, because an amendment that leaves the amended text standing is how a spine starts contradicting itself.

### AD-24 — The DJ's direct write path to `segments`: column-scoped grants + a boundary-integrity trigger `[ADOPTED 2026-08-11]`

- **Binds:** FR-14, FR-28, UX-DR9; Epic 5 Story 5.3; the `segments` table; AD-7 (the RLS shape it reuses), AD-8 (the *generic* clause it lands under — see the next bullet), AD-16 (whose "known violation" this closes), AD-23 (whose "assigned forward" hazard this closes).
- **THIS IS NOT A FIFTH AGENT WRITE, and the distinction is the point.** AD-8 admits four *named amendments* (AD-20–23) because each lets the **agent** write something. Nothing here gives the agent any capability at all: this is a DJ, in the browser, writing their own rows through PostgREST under RLS — the ordinary case AD-8's main clause already covers ("web-side mutations go through Supabase, guarded by RLS"), the same shape `sets.visibility` and `deleteSet` use. A future reader must not count this as AD-8's fifth amendment or treat it as precedent for one; an unnamed fifth *agent* write remains a violation.
- **Prevents:** a DJ-facing editor either (a) needing a bespoke RPC for what RLS already expresses, or (b) being able to write a structurally meaningless segment — one whose boundaries are reversed, whose boundary plays belong to a different set, or which overlaps another floor on the same night.
- **Rule:**
  - **INSERT and DELETE** are plain grants with `auth.uid() = dj_id` policies, AD-7's exact null-safe shape.
  - **UPDATE is column-scoped at the GRANT, not the policy**: `grant update (confirmed, type, label, first_play_id, last_play_id)`. RLS answers *whose row*; the grant answers *which columns of that row*. `set_id`, `dj_id`, `source` and `created_at` are absent from it and therefore unreachable by any DJ regardless of row ownership — a segment cannot be moved between sets, reassigned, backdated, or have its provenance rewritten. `source` in particular is immutable even to its owner, because AD-16's row-scoped overlay line and D-17's future active-learning signal both depend on provenance surviving confirmation.
  - **A `before insert or update` trigger (`segments_validate`) enforces four structural rules**, each raising a *distinct, descriptive* exception rather than a bare constraint code: boundary ordering; both boundary plays belonging to the row's own `set_id`; no overlap with another segment of the same `type` on the same set; and — temporarily — `type = 'dancefloor'` only. The four messages are a **contract**, not prose: the editor matches on them to tell a DJ which rule they hit, and `segments_write_path_test.sql` asserts each by text.
  - The `dancefloor`-only check is an explicitly **removable MVP guard** (Story 5.3 D-32/D-33), not a schema opinion — the enum has permitted `dinner`/`performance`/`custom` since Story 5.1, and this is the one thing currently narrower than it.
  - **The trigger is skipped for non-`authenticated` callers, deliberately.** `sync_set` is `SECURITY DEFINER`, so its writes run as the function owner and are exempt. Required, not incidental: Epic 5's charter forbids an overlay nicety poisoning a content sync, and a trigger able to abort that transaction would silently undo AD-23's warn-and-skip promise; and the capture-and-rebind below must be able to restore a row (post-clamp, or of a type a later story unlocks) that the DJ-facing rules would reject. The check is unforgeable from a client — reaching it any other way means already holding a privileged role — and RLS still constrains every row either way. It decides which *validations* apply, never who owns what.
- **AD-16's "known violation" and AD-23's "known hazard, assigned forward" are CLOSED by this AD** (both bullets amended in this same pass — an amendment that leaves the amended text standing is how a spine starts contradicting itself). `sync_set` now **captures DJ-authored segments whole before the plays delete and reinserts them after**, under their original `id` and `created_at`, with each boundary independently re-resolved to the play at its former `position`.
  - **Capture-and-REINSERT, not capture-and-update.** Story 5.3's own design sketched an `update segments set first_play_id = …` after the reinsert; that could never have worked, and would have failed silently. The FK cascade has already **deleted** those rows by then, so the update would match zero rows and ship the exact data loss it was written to fix, with every gate green. Recorded because the shape is counter-intuitive and the failure mode is invisible.
  - **Identity survives, which is the guarantee that actually mattered**: 5.4's segment-scoped stats and D-17's active-learning signal both need one segment to stay one segment across syncs.
  - **Per boundary, never per segment**, and **clamp, never delete**: a re-sync can invalidate one boundary while the other is still fine, and a boundary whose position no longer exists resolves to the nearest remaining one with a server-side `raise warning`. Destroying a DJ-authored row on re-sync is the disaster this exists to prevent, so it can never be the fallback. One unrecoverable case remains and is asserted as such: a set re-synced with **zero** plays has no timeline left to bind to.
- **Status:** `[ADOPTED]` — Story 5.3, 2026-08-11. Implemented in `20260811120000_add_segments_write_path.sql`; covered by `segments_write_path_test.sql`, `segments_isolation_test.sql` (whose Case 5 this story rewrote from "no write access at all" — 5.1's deliberate state, and exactly what this AD ends) and `sync_set_isolation_test.sql`'s rebind/clamp cases.
...
| Cloud mutation | Supabase/PostgREST + RLS only; agent writes only via idempotent set sync (AD-8). Five named exceptions, each scoped by a `SECURITY DEFINER` function: the Stripe webhook route handler, billing columns only (AD-18); the agent's status heartbeat, the `agent_status` row only (AD-20); the agent's library add-event batch, `library_track_events` only, first-write-wins (AD-21); the agent's library roster batch, `library_roster` only, current-state upsert on title/artist/absent_at but first-write-wins on added_at/is_baseline (AD-22); and suggested-segment materialization inside `sync_set` itself, `segments` rows with `source = 'suggested' and not confirmed` only, replaced per re-sync (AD-23). The fifth is scoped **by row**, not by table or column — it is the one exception that widens an existing write path instead of adding a new one, deliberately (AD-23). |
...
- The **session** is the immutable anchor keyed `hash(dj_id, session_identity)` (AD-16); a `set` is derived from it. `sets` carries a denormalized `derived` (jsonb) render-cache so dashboards render without recomputation; `plays` rows are the normalized record and carry `in_library`, raw + normalized genre, and `taxonomy_version` (AD-12). **Content columns (agent-written) and overlay columns (web-written: enrichment, hide, visibility) are disjoint** (AD-16); overlays are cloud-only (AD-6). **`segments` is the one qualified exception, scoped by row, not by table:** rows with `source = 'suggested' and not confirmed` are agent-derived content, written by `sync_set` (AD-23); rows a DJ authored (`confirmed = true`, or `source = 'manual'`) remain pure web-only overlay. `follows` + shared-set read policies are the Phase 2 additions (AD-15). `djs` additionally carries four additive billing columns — `stripe_customer_id`, `stripe_subscription_id`, `subscription_status`, `current_period_end` — written only by the webhook route handler (AD-18, AD-19). `agent_status` sits alongside as a 1:1 per-DJ row (PK = `dj_id`, `sync_state` + `updated_at`) carrying no Serato-derived content — written only by the agent's `set_agent_status` heartbeat RPC (AD-20), read owner-only by the dashboard.
