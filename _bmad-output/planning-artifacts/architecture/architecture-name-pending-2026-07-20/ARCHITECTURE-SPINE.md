...
- **Rule:**
  ...
  - **Known violation of the "can never wipe an overlay" claim above, scoped and assigned** *(2026-08-10, Story 5.2, D-21)*: `sync_set` deletes and reinserts a set's `plays` on every call (it has since Story 3.2), and `segments.first_play_id`/`last_play_id` carry `on delete cascade` — so a re-sync currently removes **every** segments row for that set, DJ-authored ones included. This is harmless while only recomputable suggestions exist, and becomes real data loss the moment Story 5.3 writes the first `confirmed = true` row. Fixing it means changing how plays are replaced (a stable upsert keyed `(set_id, position)`, or capture-and-rebind), which belongs to the story that creates the at-risk rows: **assigned to Story 5.3**, tracked in `deferred-work.md`. Until then the guarantee above is aspirational for `segments` and true for every other overlay.
...
- **Known hazard, assigned forward:** the plays delete+reinsert cascades through `segments`' FKs and currently wipes *all* segment rows for a set, DJ-authored ones included. See AD-16's amended bullet; assigned to Story 5.3, tracked in `deferred-work.md`.
