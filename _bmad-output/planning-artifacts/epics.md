...
> **📝 Note — more components wanted on the Library Utilization page (Arjun, 2026-08-07; unstoried).** Stories 4.3–4.5 (conversion meter, aging shelf, time-to-first-play) are not the intended final composition of that page — Arjun wants additional components beyond them. First named candidate: **# of repeat tracks between sets** (how many tracks recur across consecutive/nearby sets — a "am I playing the same thing every night" signal, the utilization-side counterpart to the aging shelf's neglect signal). Reads with **Decision B** above: repeats across *Curfew-captured* sets only, so this one is cold-start-honest by construction (it needs no play we never saw) but stays thin until ≥2 sets exist — insufficient-history state per UX-DR19. Other components TBD; capture them here as they come up, then story them before Epic 4 is called done. **[DISCHARGED 2026-08-07 — Stories 4.9 and 4.10.]** Repeat-track rate is now 4.9 AC-2, alongside set-similarity, workhorses, one-and-done and rotation size (all play-side, zero agent work); track lookup + `/track/[track_id]` detail is 4.10. The library-*side* components this note was reaching for — a nameable non-converted list, and the genre/BPM **coverage gap** — are blocked on a roster+metadata sync that the cloud does not have; that blocker is written up in full after Story 4.10 and **owned by Story 4.4**, which needs it more urgently than this page does.
...
1. **Given** a rolling window, **Then** conversion rate = % of tracks added to the library in the window that have been played ≥1 time in a set is computed. *(FR-11 — originally a fixed 90-day window, confirmed 2026-07-21, Arjun; superseded 2026-08-07, Arjun, live in the browser: default 60 days, selectable to 30 days or 2 weeks — see Story 4.3's completion notes and prd.md FR-11.)*
...
3. **Given** the metric, **Then** the active rolling-window definition is shown so the number is interpretable.
...
> **⚑ RE-SPEC per Decision B (2026-08-08, party mode + Arjun).** As originally worded, "unplayed 3+ months (from add date or last play)" measures elapsed silence against the track's real Serato add-date whenever no play has been observed. But Decision A's go-forward-only capture means a track added years before the DJ subscribed, and one they play constantly — just not yet since subscribing — would falsely surface as "aging" back to whatever its pre-subscription add-date happens to be, exactly the "reads all-aging" failure Decision B flags. Unlike Story 4.5 (fixed by excluding a population Curfew structurally can't measure), the aging shelf's whole value is surfacing a DJ's **entire** library, including tracks added long before they subscribed — excluding pre-subscription tracks would gut the feature rather than fix it. **Fix: clamp the "days unplayed" clock to `max(add_date, subscription_start_date)` instead of raw `add_date`.** A track's shelf age is never older than however long Curfew has actually been able to observe it going unplayed. A track carrying a real post-subscription last-play uses that date exactly as written today — no clamping, it's an observed fact, not an inference. Only the add-date fallback branch clamps. Copy must never expose the clamp as elapsed subscription time (no "unplayed since you joined") — state it plainly as days unplayed, per Decision B's binding copy rule.

**Acceptance Criteria:**

1. **Given** library tracks unplayed 3+ months — measured from the real last-play date when one exists, or from **`max(add_date, subscription_start_date)`** when falling back to add date — **Then** they list in the aging shelf, sortable by days-unplayed. *(FR-12, UX-DR12; clock scoped per the re-spec above)* **SHIPPED 2026-08-08 with two things this line left open, both resolved in Story 4.4:** "3+ months" is **90 days**, not three calendar months (every other Epic 4 window is a day count, and a calendar definition drifts up to 3 days by start month); and `subscription_start_date` resolves to **`djs.created_at`**, the only viable anchor — `library_roster.created_at` reads as "started watching last Tuesday" for anyone who installed before 4.11 shipped, and the earliest `sessions.started_at` does not exist for a DJ with no synced sets, who is exactly the DJ this metric is about. If that anchor cannot be read the fallback branch is **suppressed**, never degraded to raw `add_date`.
2. ~~**Given** a row, **Then** it carries an explicit "add to prep crate" action — the one place the product nudges toward action. *(UX-DR12)*~~ **RULED OUT OF MVP (2026-08-08, Arjun, story-creation session).** Shelf rows ship read-only, with **no substitute affordance** in the action's place. The finding that forced the ruling, recorded so it is not re-measured: **there is no cloud→agent command channel anywhere in this system** — AD-8 and all three of its named write amendments (AD-20 heartbeat, AD-21 add-events, AD-22 roster) are outbound-only, and nothing pulls instructions down. A real Serato crate write would additionally be the first-ever *write* to Serato (Story 2.7 scopes the agent's filesystem capability to reads), against a binary `.crate` format, with file-locking hazards while Serato runs — its own story or epic, not a task inside 4.4. Two cheaper substitutes were considered and also declined for MVP: a Curfew-side saved list in Supabase (DJ-writable via RLS, no new AD needed), and an `.m3u8` export (weak regardless — AD-2 keeps file paths off the wire, so the roster is title/artist only and Serato would have to re-match on import). **Doc-sync owed** (the ai-2/ai-6 accretion shape): PRD FR-12's UJ-6 path (`prd.md:89`), `EXPERIENCE.md`'s Aging-shelf-list Components row, and `EXPERIENCE.md` UJ-6 step 4 all still describe the action.
3. **Given** nothing is aging, **Then** the positive-framed empty state renders. *(UX-DR19 aging-shelf-empty)* **AMENDED 2026-08-08 (story-creation session) — this needs THREE states, not one.** Both clock branches are bounded below by the clamp above, so when Curfew has been observing for **less than 90 days no track can possibly qualify** — and `EXPERIENCE.md`'s aging-shelf-empty copy ("Everything you've bought is getting played.") would then be an affirmative false claim to every DJ in their first three months, which is every DJ at launch. Split into: **not yet possible** (observation < 90 days → positive-framed *wait*, UX-DR19 insufficient-history register, says nothing about whether tracks are getting played); **genuinely clear** (observation ≥ 90 days and zero qualifying → the existing copy, true only here); **nothing synced** (empty roster → the day-one empty shape every other module on this page honors, Story 4.6 AC-3).
4. **Given** recently-downloaded-not-yet-played tracks (30-day threshold — *[ASSUMPTION]; **PRD sync DONE 2026-08-08**, threshold still unconfirmed and still marked `[ASSUMPTION]` at the implementation's own constant*), **Then** the recently-downloaded nudge state renders. *(UX-DR19)* **Surface ruled 2026-08-08:** a **count line on this module**, not `EXPERIENCE.md`'s Dashboard banner — the two documents assigned it to two different pages, and a Dashboard banner is unrequested scope. Computed from **raw `add_date` with no clamp**, so it stays true even in the gated states where the shelf itself cannot judge.
5. **Given** a library track missing both `tadd` and `uadd` with no play history to fall back on (~6% of tracks, per Architecture Spine Open Questions #2), **Then** it renders in a distinct "Unknown add-date" state on the aging shelf rather than being silently omitted or defaulted into a sort position. *(Architecture Spine OQ#2 graceful-fallback requirement; SM-C1)* **Shipped as a labelled count below the list**, never interleaved into the sorted rows. A track with no add date but a real observed play is NOT in this group — its clock runs from the play, which is a fact.
6. **[ADDED 2026-08-08, Story 4.4]** **Given** a roster entry with `absent_at` set, **Then** it is excluded from every count and every list on this module — the DJ deleted it, and recommending a deleted track is the failure AD-22's soft-delete exists to prevent. *(AD-22; Story 4.11 AC-5 — this is that gate's first web-side coverage anywhere.)*
7. **[ADDED 2026-08-08, Story 4.4]** **Given** more qualifying tracks than the module renders, **Then** the cap is stated out loud — the full qualifying count and which end of the shelf is listed — never a silently truncated list that reads as the whole answer. Sorting happens **before** the cap in both directions, so ascending surfaces the genuinely shortest-aging rows rather than a reversed slice of the same capped set. *(SM-C1 "no silent caps")*
...
> **⚑ RE-SPEC per Decision B (2026-08-07, party mode + Arjun) — required before this story starts.** As originally worded, "first play" reads as the track's literal first-ever play. But Decision A's go-forward-only capture means Curfew only ever *observes* plays after the DJ subscribes — for a veteran DJ's back catalogue (added years ago, played constantly pre-subscription), the first play Curfew sees is not the track's true first play. Computing elapsed time against the real Serato add-date would read as "this track sat unplayed for three years" for a track that's been in steady rotation the whole time — precisely the "first-play-garbage" failure Decision B flags for this story. **Fix: scope the metric's entire population to tracks whose library add-date falls on or after the DJ's subscription start.** Only for those tracks can Curfew guarantee the first observed play genuinely is the track's first play — the same population-scoping pattern FR-11's conversion rate already uses (only tracks added within its window count at all). Tracks added before subscribing are excluded from this metric outright, not marked "Unknown" — "Unknown" implies missing data for a track this feature covers; a pre-subscription track isn't in this feature's domain at all. The qualifying population is thin at launch and grows as the DJ's post-subscription library accrues — the same honest-but-sparse shape Decision B gives every other Epic 4 trend. Copy must never expose the subscription-date boundary as elapsed membership time (no "tracks added since you joined Curfew") — frame it as "tracks in your library since we started watching," consistent with Decision B's binding copy rule.

**Acceptance Criteria:**

1. **Given** a track added **on or after the DJ's subscription start** and then first played, **Then** time-to-first-play = (first-play timestamp − add timestamp) is computed and displayed. *(FR-13; population scoped per the re-spec above)*
2. **Given** a track added **before** the DJ's subscription start, **Then** it is excluded from this metric's population entirely — never computed, never counted toward the aggregate, and never rendered as "Unknown" (which would wrongly imply it belongs here but is missing a value). *(Decision B — a population boundary, not a data gap)* **Scope clarification (Story 4.5 code review, ruled 2026-08-07 by Arjun):** this AC governs tracks that never enter `library_track_events` at all. It does **not** govern undated rows *inside* that table — the agent emits a track with no resolvable `tadd`/`uadd` unconditionally, so such a row may predate observation and cannot be classified either way. AC-5 governs that subset: excluded from the math, disclosed as a count.
3. **Given** a qualifying track never played, **Then** it is represented distinctly (not counted as zero).
4. **Given** the metric across qualifying tracks, **Then** an aggregate view (e.g. distribution/median) is shown, **And** a DJ with too few qualifying tracks sees the positive-framed insufficient-history state rather than a distribution drawn from a handful of points. *(UX-DR19)* **Implementation note (Story 4.5 code review, ruled 2026-08-07 by Arjun):** this needs **two** gates, not one. A qualifying-population gate satisfies the first clause but structurally cannot satisfy the second, because never-played tracks pass it and contribute nothing to the aggregate — 500 qualifying tracks with one debut would otherwise state a median drawn from n=1. A separate debut-count floor governs whether the aggregate is stated at all.
...
> **⚑ Why this story exists (Arjun, 2026-08-07, design session).** Story 4.1 shipped four metrics multiplexed behind `MetricChipToggle` into **one** chart slot, so at any moment 75% of what Epic 4 has built is hidden and the page reads as empty. Splitting the existing metrics into sections is ~4× the visible content for near-zero new computation — every series is already precomputed in `styleEvolution.ts`. The second, load-bearing reason: **the trend lines are sparse-fragile and aggregate views are not.** The page-level `monthsSpannedAll < 2` gate currently hides *everything* from a DJ with one month of history; summary tiles and (in 4.8) the aggregate charts read honestly off a single set, which is what makes this page survive the cold start Decision B keeps forcing on Epic 4.

> **⚑ RULED (Arjun, 2026-08-07): the library/digging metric MOVES to `/library-utilization`.** This closes Story 4.2's **D-11**, which was always a flagged assumption rather than a decision — the PRD groups FR-10 under Style Evolution (§4.3) while the nav naming suggested otherwise, and 4.2 shipped it as a 4th chip *pending Arjun's confirmation*. Sections forced the question; Arjun ruled the move. Rationale on the record: it is the only one of the four metrics about the DJ's *library* rather than their *playing*; Library Utilization is thin (one component, the Story 4.3 meter); and putting the conversion **trend** next to the conversion **meter** ends the risk of the two disagreeing on screen. **FR-10's PRD placement (§4.3 → §4.4) — synced 2026-08-07**, alongside this story's own FR-9 sync below.

> **📝 PRD-sync CLEARED (2026-08-07, party mode).** FR-9 described only a month-over-month trend view; it did not describe a sectioned composition or a summary-tile row — these were a post-PRD extension agreed in the 2026-08-07 design session, tracked here the same way FR-11's 90-day window and the 30-day nudge threshold were before being synced back. **FR-9** (sectioned composition + tile row) and **FR-10** (§4.3 → §4.4, per the ruling above) are now synced into `prd.md` §4.3/§4.4.
...
3. **Given** the library/digging metric (Story 4.2), **Then** it moves to `/library-utilization` and renders alongside the Story 4.3 conversion meter — the same model (`buildLibraryConversion`), the same `ConversionWindowToggle`, no recomputation — **And** Style Evolution's subtitle copy drops its "library digging" clause, **And** the trend and the meter visibly share one window selection rather than each carrying an independent toggle that can be set to disagree. *(RULED 2026-08-07; closes Story 4.2's D-11 — see the ruling block above)*
...
> **⚑ Design rationale (Arjun, 2026-08-07).** Entropy answers "how varied" but never "varied *how*" — a DJ cannot act on `2^H = 4.2`. The two charts here are the answer to that, and both are computable **today from already-synced data with zero agent work**: the genre stream reads the `breakdown: CategoryTally[]` that `styleEvolution.ts` already computes per bucket for the breakdown bars, and the Camelot wheel reads `SyncPlay.camelot_key`. The wheel is also the single biggest visual-density win available on this page: it is the geometry DJs already carry in their heads, and — unlike every trend line on the page — it looks complete off **one** set, which is exactly the cold-start property Decision B keeps costing Epic 4. Depends on Story 4.7 for the section slots it fills.
...
> **📝 Note — Style Evolution components brainstormed but deliberately unstoried (Arjun, 2026-08-07).** The design session that produced 4.7/4.8 generated more than two stories' worth. Parked here so they are not re-derived from scratch, each already checked for cold-start honesty under **Decision B** and for whether it needs agent work: **(a) genre→subgenre treemap** — `subgenre_breakdown` is synced and displayed nowhere; a treemap is the better answer than the pie chart originally proposed, since a pie fails past ~5 slices and the genre tail is long. **(b) "Your average night"** — every set's `energy_arc` aggregated onto a normalized time-into-set axis with a median curve and spread band; the DJ's signature set shape as one image. **(c) BPM ridgeline / small-multiple histograms** — the current min/max band shows range but hides *shape* (a bimodal 124+140 DJ and an all-128 DJ can plot identically). **(d) Entered/left your rotation** — genres in this bucket absent from the trailing three, and vice versa; two short lists, no chart, high narrative signal. **(e) Edit/remix detection** — **no external API needed**: `(Bootleg)`, `(Edit)`, `(VIP)`, `(Remix)`, `vs.`, `ft.` are pattern-detectable off `SyncPlay.title`/`artist`, giving "% of your set was edits" for free; ships as *detected* edits with the false-negative caveat stated (untitled personal edits read as originals), never as a claimed total. **(f) Billboard / Spotify chart cross-referencing** — **explicitly post-MVP**: fuzzy title-artist matching against chart data is its own project, Billboard has no clean licensed feed, and it answers a crowd-facing question ("how mainstream am I") rather than a style-evolution one; revisit as a Phase-2 "crowd-pleaser" concept. **(g) "When am I likely to play this track"** — deferred on statistics, not effort: go-forward capture means most tracks have 1–2 plays, so a play-time distribution is noise at launch; the honest cheap cousin is a clock-time dot strip on **Set Detail** for tracks with 3+ plays. **(h) Average tracks/hour** — subsumed by 4.7's **mix pace** tile, which is the same quantity inverted; ruled to Style Evolution not Library Utilization on the dividing line agreed this session: *Style Evolution = how you play; Library Utilization = what you own and whether you use it.* By that same rule **repeat-track rate stays on Library Utilization**, matching the existing note at the top of this epic — now storied as **4.9 AC-2**.
...
> **⚑ Why this story exists (Arjun, 2026-08-07).** Library Utilization holds one component (Story 4.3's meter) and receives a second from Story 4.7 AC-3 (the moved conversion trend). This story is the page's actual composition plus the batch of utilization stats that need **no agent work at all** — every one is computable from `plays` alone. It discharges the epic-header note's "more components wanted… first named candidate: # of repeat tracks between sets," and that note's "capture them here as they come up, then story them" instruction. Deliberately paired with Story 4.10 (track lookup), and deliberately **excluding** the library-side stats that need a synced roster — see the prerequisite note after 4.10.
...
3. **Given** repeat-track rate needs a definition that cannot drift, **Then** "nearby" means a **trailing 5-set window** (ruled 2026-08-07, party mode: wide enough to be forgiving of gaps between gigs without reading as counting ancient repeats) and the surfaced quantity is named as precisely as Story 4.1's amended AC-5 names 2^H — never left as "a rate" for the implementation to settle. *(4.1's D-10 lesson: a spec that described something other than what shipped)*
   > **⚑ RULED 2026-08-08 (Arjun) — the quantity, written here so it is not re-derived from the story file.** For each surviving set with at least one surviving predecessor: `(distinct tracks in that set that also appear in the up-to-5 surviving sets immediately preceding it) ÷ (distinct tracks in that set)`. The surfaced figure is the **unweighted mean** of those per-set shares, with **the count of sets it averaged over stated alongside it**. The 5 are **predecessors only** — the measured set is never one of its own window. The oldest surviving set has no predecessor and is **excluded from the mean, never counted as 0%** (a gap, never a fabricated zero). *Unweighted is deliberate:* AC-2 asks whether a typical **night** repeats, so each night counts once regardless of length; a play-weighted mean would answer a different question and let one marathon outvote a month of shorter gigs. **This ruling and AC-10's predicate must ship together or not at all** — the small-set skew an unweighted mean would otherwise suffer is removed precisely by the compound predicate below. A single-ratio alternative ("of the distinct tracks across the trailing 5 sets, the share appearing in more than one") was considered and **rejected**: it cannot skew, but it reads only the most recent block and so cannot answer "does a *typical* night repeat." Do not implement both, and do not average them. With exactly 2 surviving sets there is exactly 1 measurable set — which is why AC-2's gate is "≥2 captured sets" and not "≥2 measurable ones." *(Implemented in Story 4.9 as D-17; `web/lib/sets/libraryUtilization.ts` → `buildRepeatTrackRate`. Story is in `review` as of 2026-08-08 — not yet marked done.)*
...
10. **Given** low-confidence sets, **Then** this page applies the same exclude-**visibly** contract Style Evolution does — soundchecks must not silently inflate a repeat rate. *(FR-27; Story 4.1 AC-2)*
    > **⚑ RULED 2026-08-08 — the shared thing is the AFFORDANCE, and the predicate deliberately DIVERGES.** "The same contract Style Evolution does" selects Style Evolution's bare `confidence.value < 1.0`, and **that predicate does not do what this AC exists for.** `agent/src-tauri/src/confidence.rs` is symmetric by its own design note — *"A session that's obviously 'not a set' (too few plays) and a session that's obviously a real set both score `1.0`"* — so the low-confidence value is only ever assigned to the dense-continuous ambiguous case, and **a two-track soundcheck scores a clean 1.0** and sails straight through. This page therefore uses `listModel.ts`'s **compound** `isLowConfidenceSet` (`confidence.value < 1.0 || track_count < HERO_MIN_TRACKS`), whose `||` term exists precisely to catch that case — `rightColumn.ts` records the shipped bug it was added to fix (a one-play soundcheck crowning the most-played card). What is shared with Style Evolution is the *exclude-visibly affordance* (hidden by default, count disclosed, reveal offered), not the predicate. **Do not "reconcile" this page onto the bare form.** Two consequences: the contract applies **page-wide** (the meter, the trend and time-to-first-play read the same surviving population under one reveal — a control that governed only some of the figures beneath it would misdescribe its own scope); and the reveal's copy had to widen to *"N short or low-confidence sessions hidden"*, because under the compound predicate the shipped noun was wrong. Three disagreeing definitions of "low confidence" exist in `web/` (`styleEvolution.ts`, `listModel.ts`, `SetHeader.tsx`) — each now carries a comment naming the other two; reconciling all three is its own story. *(Implemented in Story 4.9 as D-20. Story is in `review` as of 2026-08-08 — not yet marked done.)*
...
> ~~**⚑ Scope boundary — this searches PLAYED tracks, not the library, and the copy must not pretend otherwise.** The cloud holds full metadata for every played track (`SyncPlay` carries title, artist, `bpm`, `genre.normalized`/`subgenre`, `camelot_key`, `played_ms`, `library_added_at`, plus set and position), so the **detail view is complete today with zero agent work**. What is *not* available is the library roster: D-1's first-run baseline is never synced, and `SyncLibraryAddEvent` carries only `{ track_id, added_at }` — a hash and a date, no metadata. So a track the DJ owns but never played can be neither found *nor named*. The sequencing follows from that asymmetry: a never-played track's detail page would be nearly empty anyway, so build the rich half now and widen **discovery** when the roster lands (see the prerequisite note below).~~
>
> **⚑ SUPERSEDED — RULED 2026-08-10 (Arjun), implemented as Story 4.10's D-25. Search covers played ∪ owned.** The blockquote above was written before the roster existed and its premise expired on 2026-08-08, when **Story 4.11 shipped `public.library_roster`** (`track_id`, `title`, `artist`, `added_at`, `is_baseline`, `absent_at`). Its own **AC-9 assigns the consequence to this story** — *"Story 4.10's search widens from played tracks to the full library and its AC-2 empty-state copy is revisited"* — so the later, shipped ruling governs and **AC-1/AC-2 below are amended accordingly**. The asymmetry the note reasoned from is genuinely gone: an owned-but-unplayed track can now be both found *and* named. What survives from it is the honesty requirement, inverted — the label must name **both** populations, because "your library" alone over-promises on the ~28% of catalogue rows with no resolvable identity, and "tracks you've played" alone is now simply false. A result therefore carries which population it came from. This also turns out to be the product's best cold-start answer: a DJ with a synced roster and zero sets can search and find tracks they own on day one (D-38).

**Acceptance Criteria:**

1. *(AMENDED 2026-08-10, D-25 — supersedes the struck note above.)* **Given** the Library Utilization page, **Then** a search field finds tracks by title and artist across **both the tracks the DJ has played and the tracks in their synced library**, and its label states exactly that — never "your library" alone (which over-promises on the ~28% of catalogue rows with no resolvable identity), never "tracks you've played" alone (which is now false). *(Story 4.11 AC-9)*
2. *(AMENDED 2026-08-10, D-25.)* **Given** a result, **Then** it states which population it comes from — played (with its play count) or owned-but-not-yet-played (with its add date) — so the DJ never has to guess why a track shows no plays. **Given** a query with no match, **Then** the empty state says plainly that Curfew has no record of that track in **either** population, without apology and without implying the library is complete. The ambiguity the original AC existed to manage is the one Story 4.11 removed. *(UX-DR18 calm failure register)*
3. **Given** a result **that has a resolvable identity**, **Then** it opens a track detail route at `/track/[track_id]`, mirroring the existing `/set/[id]` route convention rather than inventing a second pattern. *(Story 3.7 precedent)*
   > **⚑ RULED 2026-08-10 (Arjun) — implemented as D-26. A fifth of the DJ's plays cannot have a detail page, and that is AD-11 working, not a bug to route around.** `plays.track_id` is nullable and Story 4.3's identity change made it structurally so: `track_id_from_title_artist` requires **both** title and artist. Measured, **473 of 2,294 real plays (~21%) carry no `track_id` at all**, and on the committed seed **212 of 1,267 distinct tracks** have none. Two full-coverage alternatives were costed and declined: keying the route on `trackKey` (title+artist) produces encoded URLs, contradicts this AC, and inherits that key's un-trimmed/un-case-folded form so `"Deep End "` becomes a second page; a hybrid falling back from id to key puts two URL shapes and a third identity usage on a page that already deliberately runs two (Story 4.9's D-18). **So such rows stay readable, render unlinked rather than as dead links, and their count is disclosed on the page in one sentence** — a new AC-4 in the story file. Do not invent a title-only identity to paper over it: AD-11 ruled one field is too little signal, and Story 4.3 paid a measured 21% coverage cost to honour that.
4. **Given** the detail view, **Then** it shows identity and tags — title, artist, BPM, Camelot key, genre → subgenre — sourced from the play rows, with any absent field rendered per FR-2's **Unknown** convention rather than blank or guessed. *(FR-2; AD-11)*
5. **Given** `library_added_at` is present, **Then** the detail view shows when the track entered the library; **Given** it is absent (the ~6% no-`tadd`/`uadd` case, or a track acquired before Curfew), **Then** it renders as a distinct honest state — never guessed, never defaulted to the first play. *(Architecture Spine OQ#2; Decision B; AD-11)*
6. **Given** play history for the track, **Then** the detail view shows times played, first and last play, and the count of distinct sets — **And** lists those sets as rows linking into `/set/[id]`. *(FR-9 family; Story 3.7)*
7. **Given** the track's plays, **Then** a **clock-time strip** shows what time of night the DJ drops it. This is the deferred "when am I likely to play this" idea, viable **here and only here**: scoped to one track, n=4 is readable, whereas as a population statistic go-forward capture makes it noise. *(Epic-4 unstoried note item (g), now homed)*
   > **⚑ GAP CLOSED 2026-08-10 — implemented as D-32. "What time of night" has no timezone, and nothing in the system stores one.** `plays.started_at` is `timestamptz`: the capture-side offset is normalized to UTC and lost, and there is no venue timezone, no DJ timezone on `djs` and no set-level offset anywhere. Rendering an hour server-side renders it in the **server's** zone — wrong for every DJ not sitting in it, and a hydration mismatch besides (this epic already carries one unfixed instance, and 4.7 shipped another). **So the model ships `startedAtMs: number` and never a formatted hour; the labels and the strip's text equivalent are both produced in the browser, in the viewer's own zone** — which is the only zone ever right for this question, since the DJ plays where they live. Note that `vitest.config.ts` pins `TZ=UTC`, so a green unit test proves the epoch math and nothing about the rendered hour; that is verified in the browser pass instead (done 2026-08-10: plays stored at 15:00 UTC rendered as **11am** under `America/New_York`).
8. **Given** `played_ms` (~98% populated per Story 3.7 §3d), **Then** the view shows how long the DJ typically rides the track, **And** plays missing `played_ms` are excluded with their count disclosed — never folded in. *(SM-C1; the `no_genre_count` "never omitted" discipline)*
9. **Given** the track's position in each set, **Then** **mix neighbours** render: what was played immediately before and after it, across all plays, ranked by recurrence. Computable today from `position` alone; nothing else in the product answers it. *(New)*
10. **Given** a track played exactly once, **Then** every aggregate above degrades honestly to its n=1 form — a single dot, a single neighbour pair, no median presented as though it were a distribution. *(D-8's "a gap, never a fabricated value")*
11. **Given** low-confidence sets, **Then** the detail view applies the same exclude-**visibly** contract as the rest of Epic 4 — a track's play count must not be silently inflated by soundchecks, nor silently reduced without the DJ being told. *(FR-27; Story 4.1 AC-2)*
12. **Given** search and the detail view, **Then** both are fully keyboard-operable, results are announced to assistive tech, the clock-time strip and mix-neighbour lists carry text equivalents, and 375px/320px are verified against the DOM. *(UX-DR21, UX-DR22; 4.1's review lesson)*
...
> **⚑ Sequenced BEFORE Story 4.4, as its own story rather than inside it.** It has **three** consumers, not one — 4.3's meter (which can say 34% converted but cannot show the 66%), 4.4's aging shelf (which must render named rows with a prep-crate action and would otherwise render hashes), and 4.10's search (limited to played tracks without it). A prerequisite serving three consumers should not live inside one of them. *(Supersedes this note's earlier "Owner: Story 4.4" assignment, made before the cost was measured.)*

> **⚑ Cost measured 2026-08-07, and it is asymmetric — this is the story's central decision.** **Tier A (title + artist): nearly free.** `DateAddedIndex::all_tracks()` already returns `(portable_path, date_added, title, artist)` for the whole library, and `capture::scan_library_adds` **discards `title`/`artist` one line after hashing them into `track_id`** (`capture.rs:267`). Carrying them costs one `filter_map`, two columns, and the contract/cloud/web carry. This tier alone unblocks all three consumers above. **Tier B (BPM, key, genre): a real whole-library enrichment pass.** Those fields come from the joiner's per-track lookup (`JoinedMetadata`), not from `all_tracks()`, so covering unplayed tracks means enriching the entire catalogue against NFR-1's ≤10 s p95 full-library budget. Tier B buys exactly one thing: the **coverage-gap** components. **Recommendation: ship Tier A here; park Tier B with coverage-gap.**

**Acceptance Criteria (Tier A):**

1. **Given** the library scan, **Then** `title` and `artist` are persisted on `library_tracks` alongside the existing `track_id`/`added_at`/`is_baseline`, rather than discarded after hashing. *(`capture.rs:267`)*
...
9. **Given** the new roster, **Then** Story 4.10's search widens from played tracks to the full library and its AC-2 empty-state copy is revisited — the ambiguity that AC exists to manage is exactly what this story removes. *(Story 4.10 AC-1/AC-2)*
   > **✅ DISCHARGED 2026-08-10 by Story 4.10 (D-25).** Search now covers played ∪ owned, each result states which population it came from, and 4.10's AC-1/AC-2 are amended above. The pre-roster scope-boundary blockquote at the head of Story 4.10 is struck through with the ruling attached.

> **⚑ PREREQUISITE DETAIL — retained for context; superseded as to ownership by Story 4.11 above.** Verified 2026-08-07 against `shared/src/index.ts` and `agent/src-tauri/src/store.rs`: the agent's local `library_tracks` table holds the whole library including the D-1 baseline, but stores **`track_id` + timestamps only** — no title, artist, BPM, key, or genre — and baseline rows are structurally unreachable from the sync queue (`WHERE is_baseline = 0`). `SyncLibraryAddEvent` likewise carries only `{ track_id, added_at }`. **Consequence the epic has not recorded: the cloud can COUNT an added-but-never-played track but can never NAME one.** That is a *second, distinct* blocker on **Story 4.4** — separate from the last-play cold-start problem Decision B already flagged — because the aging shelf must render named rows with a prep-crate action, and even restricted to post-install tracks it would render a list of hashes. It equally caps **Story 4.3's meter** at a number the DJ cannot act on (34% converted, but the 66% cannot be shown). **Shape of the fix:** persist metadata alongside the local roster (the joiner already reads BPM/key/genre from the Serato library for enrichment, so the data is reachable), an additive `shared/` type (AD-3/AD-15), a cloud table with RLS matching `library_track_events`, and carry via the Story 3.3 queue — the **third** sanctioned amendment to AD-8's "agent's only write" rule after AD-20 (heartbeat) and AD-21 (library events), so it must be proposed deliberately, not landed quietly. **Two things it does NOT do:** it does not un-break 4.4/4.5's Decision B problem (it fixes *which tracks exist*, not *plays Curfew never saw* — a veteran's track played 100× pre-install still reads "never played"), and it would be the first time the cloud holds a full inventory of the DJ's library rather than only what they played, which **must be checked against Story 2.7's local-only raw-data boundary** before it is assumed acceptable. **What it unlocks once done:** real library-wide search in 4.10, a nameable non-converted list for 4.3, and the **coverage-gap** components (library genre/BPM mix vs *played* mix as diverging bars — "you own 30% house, you play 12%"), which are the most definitionally-utilization stats available to this page and are parked here against it.
...
> **⚑ Design session — algorithm designed (Arjun, 2026-08-10). Authoritative spec: `implementation-artifacts/5-2-segment-detection-algorithm-design.md`** (design working doc; read it before creating/deving this story; renamed from `5-2-segment-detection-algorithm.md` at story creation so the dev-ready story file could take the canonical story-key path). Headlines: one shared algorithm for every DJ, ported to the **agent stat-engine (Rust)**, not the client; v0's global constants become the **shared cold-start prior** every DJ's calibration blends from (no hard cutoff, no separate "no detection until N sessions" period); density+BPM stay the candidacy gate exactly as v0 shaped it, smoothness becomes a **confirming gate** evaluated only on already-qualifying runs; **all three floors** (density, BPM, and the new smoothness signal) are per-DJ percentile-calibrated — smoothness being per-DJ, not a fixed constant, is what absorbs club-DJ-vs-wedding-DJ variance without ever encoding a DJ "type"; smoothness is computed as a **window-level aggregate**, not per-pair, so an isolated request-drop doesn't fail an otherwise-tight window while a real end-of-night announcement cluster (multiple irregular tracks in a row) still trips it; idle/gap is a descriptive label layered on a density-floor miss, not its own confirmation gate, but gets its own (shorter) merge/bridge tolerance distinct from the ordinary single-window bridge; a qualifying run simply extends to the last play in the data, so a set ending mid-peak needs no special-case "end" signal. Also resolved: the `segments.source`/`status` column shape 5.1 deferred to this story — **two orthogonal columns** (`source {suggested, manual}` + a `confirmed` boolean, with a CHECK ruling out the one impossible cell), not a collapsed enum, because provenance needs to survive confirmation for the future active-learning signal below. Left genuinely open (see the doc's §5): exact percentile/threshold values, window size, and idle's precise definition. Also out of scope, deliberately: durable per-DJ calibration-profile storage (leaning Story 5.3's territory) and any edit-feedback learning loop (DJ boundary corrections reshaping calibration) — both flagged forward, not designed here.
...
> **⚑ Design session (Arjun + installed-agent room, 2026-08-11). Authoritative spec: `implementation-artifacts/5-3-segment-editor-design.md`** (all decisions locked, nothing left open). Headlines: **AC #4 narrows for this story's MVP to dancefloor-only** — the DJ can create/edit/confirm dancefloor segments; dinner/performance/custom typing and custom labels are deferred to a later story, not cut from the product (the schema enum stays as-is; a light, removable DB guard enforces the narrower MVP write scope in the meantime). The launch blocker discovered in 5.2 (D-21 — every `sync_set` re-sync wipes all `segments` rows via the `plays` delete+reinsert cascade, destructive the moment this story writes a `confirmed`/`manual` row) is fixed by **capture-and-rebind** *(shipped 2026-08-11 as capture-and-**reinsert** — the correction is load-bearing: the `on delete cascade` this hazard is made of has already removed the segments rows by the time any rebinding UPDATE could run, so the row must be captured whole and written back under its original `id`/`created_at`, not merely re-pointed. An UPDATE-based rebind would have matched zero rows and shipped the exact data loss it was written to fix, with every gate green. See AD-24.)*: each affected boundary resolves to its `position` before the delete and rebinds to the new `plays.id` at that position after reinsert, per-boundary (not per-segment, since a segment's two edges can go stale independently), inside `sync_set`'s existing transaction; a boundary whose position no longer exists after a shrinking resync clamps to the nearest valid one and logs a warning rather than ever silently deleting DJ-authored data. Write policy: UPDATE-in-place for drag-adjust/confirm/relabel (preserves segment identity across an edit), INSERT for a new manual boundary, DELETE for removal — new `dj_id`-scoped RLS grants, with a **column-level** UPDATE grant restricting which fields are mutable (`set_id`/`dj_id`/`source`/`created_at` stay immutable post-creation). A single trigger closes three previously-deferred gaps at once: boundary ordering, FK/set consistency (a segment's plays must belong to its own `set_id`), and no same-type overlap between segments in one set. The interaction model: boundaries render as handles **between** tracklist rows (not on a row), with a rail marking the bracketed range; the energy arc mirrors that state rather than hosting an independent second drag target; suggested and confirmed segments get genuinely distinct visual treatment; keyboard access is built fresh with `aria-valuetext` (track name + timestamp, not a bare number) and live-region nudge announcements, since no arrow-key/slider precedent exists anywhere in this codebase yet; **primary touch interaction is tap-to-mark, with drag as a desktop-only enhancement layered on top** (Arjun's call — simplifies build order rather than complicating it). Also resolved: durable per-DJ calibration-profile storage, provisionally flagged toward this story by 5.2's design doc, turned out to already be resolved by 5.2 as a live runtime computation — not owed here after all. A bare multi-segment selector (which of possibly-several dancefloor segments is being edited) is this story's floor; full cross-segment comparison stays Story 5.4's.
...
> **⚑ Refinement (Arjun, 2026-08-11 — MVP is dancefloor-only for launch; authored story: `implementation-artifacts/5-4-segment-scoped-stats.md`).** Story 5.3's D-32/D-33 already narrowed the *editor* to dancefloor-only segments for MVP (`dinner`/`performance`/custom types are schema-ready but write-guarded off); this story's original "compare, say, the dinner hour to the peak dancefloor" framing assumed those other types exist, so it narrows the same way, for the same reason. This story's real MVP shape: a set can carry **zero, one, or several dancefloor segments** (AR-13 — a wedding's cocktail-hour floor and post-dinner peak are two, not one), and today every consumer (Set Detail's scope toggle, the dashboard set-list row, the hero band) silently collapses to the single **longest** one via `dancefloorSegments.ts`'s `primaryDancefloorSegment` — an interim rendering pick recorded as such (D-24) and flagged as a real, live gap in `deferred-work.md` ("A set with several dancefloor segments renders only its longest, everywhere, with no affordance saying so" — 15 of 58 sample sets hit this today). This story is that gap's owner (per 5.3's own D-30: "full cross-segment comparison stays Story 5.4's" — 5.3 ships only a bare editing-target selector, not a view-scoping one). AC #1 is reinterpreted as: **cross-dancefloor-segment comparison** within one set (pick which dancefloor scopes Set Detail's stats when 2+ exist), not cross-type comparison — dinner/performance/custom comparison stays deferred until a later story unlocks those types in the editor. AC #2's "cloud may re-aggregate" stays permissive, not mandatory: Story 3.7 already established client-side recompute from the already-fetched `plays[]` (`scopedPlays`/`frame`) as the shipped pattern for whole-vs-dancefloor scoping, and this story extends that same pattern to per-segment scoping rather than standing up a parallel cloud SQL path AR-8 never required.

### Story 5.5: Layer 2 enrichment form

> **⚑ Refinement (Arjun, 2026-08-11 — where the form actually lives).** Story 5.3 shipped an explicit **edit mode**: selecting a dancefloor chip pins a sticky action bar (Confirm / Cancel / Remove), dims the tracks outside that floor, reveals the gutter "+", and scrolls to the floor's first boundary. **That edit mode is the host for Layer 2's fields** — venue, crowd, event type, notes and photos belong in the same pinned panel, entered against the segment the DJ has already selected, not on a separate screen or a second surface. This sharpens the 2026-08-02 refinement ("this editor and 5.5's enrichment share one Set Detail editing surface") from a statement about the *page* into one about the *mode*: the surface already exists, is already stateful, already knows which floor is active, and already has a commit/cancel contract to extend. 5.5 adds fields to a panel; it does not build a panel.
>
> Two consequences worth stating now, so 5.5 does not re-litigate them: enrichment is **set-scoped today and segment-scoped the moment it lands here** — if the fields sit inside a per-segment edit mode, "which floor was the packed one" becomes answerable, which is more than the original AC asked for and should be a deliberate choice rather than an accident. And `segments` currently has no columns for any of it, so 5.5 owns that additive migration plus whatever RLS/column-grant shape mirrors AD-24's (the column-scoped UPDATE grant will need widening, which is exactly the kind of change AD-24's grant list was written to make explicit).
