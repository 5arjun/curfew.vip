...
> **⚑ BLOCKED — read the prerequisite note after Story 4.10 before starting this story.** Two *separate* problems sit on this story, and only one of them is Decision B's. (1) The known one: "last play" is go-forward-unknowable, so a veteran's played catalogue falsely reads all-aging — Decision B already requires a re-spec (restrict to post-install tracks and/or gate until warm-up). (2) **The one found 2026-08-07 and not previously recorded: the cloud can count an added-but-never-played track but cannot NAME it.** `SyncLibraryAddEvent` is `{ track_id, added_at }` — a hash and a date — and the agent's local roster carries no metadata either. This story's ACs require named rows with a prep-crate action; as things stand it would render a list of hashes, *even after* fixing (1). Fixing (2) is **Story 4.11**, which must land before this story starts. Do not treat the Decision B re-spec as sufficient to unblock this.
