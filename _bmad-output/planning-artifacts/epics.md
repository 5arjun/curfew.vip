...
1. **Given** library tracks unplayed 3+ months — measured from the real last-play date when one exists, or from **`max(add_date, subscription_start_date)`** when falling back to add date — **Then** they list in the aging shelf, sortable by days-unplayed. *(FR-12, UX-DR12; clock scoped per the re-spec above)*
