...
1. **Given** the Library Utilization page, **Then** a search field finds tracks by title and artist across the DJ's played history, and its label states what it searches — **"tracks you've played"**, never "your library." *(Scope boundary above)*
2. **Given** a query with no match, **Then** the empty state resolves the ambiguity the DJ is otherwise left holding: no result means Curfew has no record of *playing* it, which is not the same as it being absent from the library — said plainly and without apology. *(UX-DR18 calm failure register; the honesty gap this feature would otherwise open on a page called Library Utilization)*
3. **Given** a result, **Then** it opens a track detail route at `/track/[track_id]`, mirroring the existing `/set/[id]` route convention rather than inventing a second pattern. *(Story 3.7 precedent)*
