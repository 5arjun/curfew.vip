...
- [x] **Task 4 — Unit tests: synthetic inputs, one per AC + determinism** (AC: 1, 2, 3)
  ...
  - [ ] ⚠️ **NOT RUN — no Rust toolchain on the implementation machine.** Full crate gate stays green: `cargo fmt --manifest-path agent/src-tauri/Cargo.toml -- --check`, `cargo clippy --manifest-path agent/src-tauri/Cargo.toml --all-targets -- -D warnings`, `cargo build --manifest-path agent/src-tauri/Cargo.toml`, `cargo test --manifest-path agent/src-tauri/Cargo.toml`. This box has no `cargo`/`rustc`/`rustup` installed (prior stories were gated from a macOS machine), so the four commands could **not** be executed. Code was written to pass by careful hand-review; **CI's `agent` job (or any toolchained machine) must run this gate before merge.** See Completion Notes + `deferred-work.md`.
...
- **11 inline tests** authored covering AC-1 (bucket + version stamp + verbatim raw), AC-1 matching (case/whitespace variants), canonical-name self-map, AC-2 (default bucket), the missing-vs-unrecognized `None` distinction, blank policy, the `"(17)"` hand-off, determinism, a version-stamp guard, and two table-integrity guards (no shared alias, all aliases lowercased/trimmed). **These tests are written but UNVERIFIED — see Debug Log.**

⚠️ **Definition-of-Done caveat:** the "all tests pass / gate green" DoD item is **not** satisfied on this machine. Everything else (implementation, AC coverage by construction, File List, docs, scope) is complete. A reviewer/CI with a Rust toolchain must run the four-command gate before merge.
