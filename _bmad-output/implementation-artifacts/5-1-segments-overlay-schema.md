...
- [ ] Task 1: Write the migration (AC: #1, #2, #3)
  - [ ] 1.1 `supabase migration new create_segments` → `supabase/migrations/<timestamp>_create_segments.sql`
  - [ ] 1.2 Create `public.segments` per the exact schema in Dev Notes below (`id`, `set_id`, `dj_id`, `type`, `label`, `first_play_id`, `last_play_id`, `created_at`)
  - [ ] 1.3 Enable RLS; add the `dj_id`-owner SELECT policy (AD-7 pattern), named `segments_select_own` to match `sessions_select_own`/`sets_select_own`/`plays_select_own`
  - [ ] 1.4 **In the same migration**, explicitly `revoke all on public.segments from anon, authenticated;` before granting SELECT — see Dev Notes "The hosted auto-expose trap" for why this is not optional even though RLS is enabled and no write grant is intended
  - [ ] 1.5 `grant select on public.segments to authenticated, anon;`
  - [ ] 1.6 Do **not** add any INSERT/UPDATE/DELETE grant or policy in this migration
- [ ] Task 2: Extend the generic grant-matrix sweep (AC: #1)
  - [ ] 2.1 Add `'segments'` to the two hardcoded 7-table arrays in `supabase/tests/grant_matrix_test.sql` (the `anon`/`authenticated` INSERT-denied sweep at line 30 and the `anon` DELETE-denied sweep at line 39) — the TRUNCATE sweeps and the two catalog-driven generic sweeps at the bottom of the file already cover any new table automatically, but the INSERT/DELETE checks are hardcoded lists and will silently skip `segments` unless added
  - [ ] 2.2 Bump `select plan(51)` to `plan(53)` (two new rows land in the now-8-table INSERT and DELETE `unnest` sweeps) and update the file's leading plan-count comment
- [ ] Task 3: Verify additive-only + isolation locally (AC: #1, #2, #3)
  - [ ] 3.1 `supabase start` then `supabase migration up` — confirm the migration applies cleanly
  - [ ] 3.2 `supabase/scripts/check-additive-only-migrations.sh supabase/migrations` — confirm it passes
  - [ ] 3.3 Manually insert a `segments` row with `type = 'custom'` and no `label` — confirm the CHECK constraint rejects it; insert one with a `label` — confirm it's accepted
  - [ ] 3.4 Manually insert a `segments` row with an out-of-enum `type` — confirm the CHECK constraint rejects it
- [ ] Task 4: Write the pgTAP isolation test (AC: #1, #2, #3)
  - [ ] 4.1 Create `supabase/tests/segments_isolation_test.sql`, mirroring `supabase/tests/sessions_sets_plays_isolation_test.sql`'s structure (seed two DJs via `auth.users`, seed one `sessions`/`sets`/`plays` row chain per DJ, seed a `segments` row referencing each DJ's own `plays` rows)
  - [ ] 4.2 Cover: cross-DJ SELECT isolation both directions; `anon` sees zero `segments` rows; `authenticated` has zero write access (`throws_ok` against `42501` for INSERT/UPDATE/DELETE, matching Case 5's shape in the 3.1 suite)
  - [ ] 4.3 Cover negative-path constraint coverage (matching 3.1's Case 1c/1d/1e/1h/1i shape): out-of-enum `type` rejected (`23514`), `type='custom'` with null `label` rejected, a dangling `first_play_id`/`last_play_id` reference rejected (`23503`)
  - [ ] 4.4 Cover cascade: deleting the referenced `auth.users` row removes the DJ's `segments` row too (mirrors 3.1's Case 7); deleting the referenced `plays` row removes the `segments` row that pointed at it (proves the `on delete cascade` from Ruling 1 — see Dev Notes)
  - [ ] 4.5 `supabase test db supabase/tests` — confirm the full suite (including the updated `grant_matrix_test.sql`) passes
- [ ] Task 5: Close the two account-deletion runbook forward-hooks (AC: #1)
  - [ ] 5.1 `supabase/ACCOUNT-DELETION-EXPORT-RUNBOOK.md` §2 already names this story and states the cascade *should* already be covered by its ruling — confirm that for real against local Postgres (not assumed from the DDL), same discipline Story 3.1 used on itself, and update the note from "should" to "confirmed" once verified
  - [ ] 5.2 §3's export query list already has the `segments` line added (`select * from public.segments where dj_id = '<uuid>';`) — no further edit needed, just confirm it's present
- [ ] Task 6: Update `supabase/README.md`'s migration tree map (AC: #1)
  - [ ] 6.1 Add the new migration filename to the `migrations/` block in the top-of-file tree, matching the existing entry format (`  <filename>  # <one-line> (Story 5.1)`)
  - [ ] 6.2 Add `segments_isolation_test.sql` to the `tests/` block the same way
