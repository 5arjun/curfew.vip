...
- [x] [Review][Defer] `first_play_id`/`last_play_id` aren't constrained to belong to the same `set_id` as the `segments` row — a segment could reference plays from a different set [supabase/migrations/20260810153813_create_segments.sql:19-29] — deferred, pre-existing pattern: Story 3.1's own migration explicitly documents the identical "known gap, not this story's to close" for `dj_id`-vs-parent consistency; enforcement belongs to the future write-path story (5.2/5.3), which must derive rather than trust these values, matching ~~AD-19's pattern~~ **[citation corrected 2026-08-11, Story 5.3: AD-19 is the subscription-billing AD (`ARCHITECTURE-SPINE.md:184`) and has nothing to do with this; no AD in the spine used "derive, don't trust" phrasing at the time this was written. The principle was real, the citation was not — it propagated verbatim into `deferred-work.md`'s copy of this entry. It is now carried for real by **AD-24**, and the gap itself is CLOSED by that AD's `segments_validate` trigger (D-29.2).]**
...
- **`on delete cascade` on both FKs is deliberate**, not a default nobody looked at: if a future format-drift backfill (Story 3.4's territory) ever needs to delete/replace a `plays` row, a `segments` row still pointing at it is meaningless and should die with it rather than silently dangle. `plays` rows are not currently deleted by any shipped story — this is a forward-looking guard, not a response to an observed failure.
  ...
  > **Correction, 2026-08-10 (Story 5.2 dev — D-21).** The last sentence above is **false, and has been since Story 3.2.** `sync_set` does `delete from public.plays where set_id = ... ` followed by a full reinsert on **every** call, so play UUIDs churn on each re-sync and this cascade fires routinely — it is not forward-looking at all, it is live today. The *reasoning* for the cascade still holds; only the premise about when it fires was wrong.
  >
  > The consequence this premise was hiding: because the cascade fires on every re-sync, a re-sync currently removes **every** `segments` row for that set. Harmless for Story 5.2's `('suggested', false)` rows, which are meant to be recomputed (and whose deletion `sync_set` now makes explicit rather than cascade-incidental). **Destructive for the `confirmed = true` / `source = 'manual'` rows Story 5.3 will create** — a queue redelivery, a backfill sweep or a Serato re-save would silently destroy DJ-authored overlay, violating AD-16's "an idempotent re-sync can never wipe an overlay." Assigned to Story 5.3 with a `deferred-work.md` entry; AD-16 and AD-23 in the Spine now scope the guarantee accordingly.

- **Correction, 2026-08-10 (Story 5.2 dev): this story's migration header describes 5.2's algorithm as running cloud-side; it does not.** `20260810153813_create_segments.sql` says "Story 5.2's detection algorithm running cloud-side" in its opening comment and "Story 5.2 (algorithm-written suggestions, cloud-side)" in its trailing one. Locked design decision **D-2** puts detection in the **agent's Rust stat engine** (`agent/src-tauri/src/stats/segments.rs`), where AR-13, epics.md and AD-17 always said it would live; what happens cloud-side is only the *write materialization* — `sync_set` resolving positions and inserting the rows (D-19/AD-23). Migration files are immutable once committed, so the correction lives here and in AD-23 rather than in the file itself.
