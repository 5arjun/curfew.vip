...
### Review Findings — backend group

Code review of the backend group (`agent/src-tauri`, `shared/`, `supabase/`) via `bmad-code-review`, 2026-08-04 — the follow-up run the frontend review above deferred. Blind Hunter + Edge Case Hunter + Acceptance Auditor run in parallel against the diff vs `main`; findings read against the actual code (not just diff hunks) before rating, per this project's own repeated lesson that automated findings need verification.

**Decision-needed** — resolved by Arjun, 2026-08-04

- [x] [Review][Patch] `backfill_captured_serato4` can't distinguish "source unreachable" from "every row re-derived to an unplayed preview" — [agent/src-tauri/src/backfill.rs:169](../../agent/src-tauri/src/backfill.rs#L169), [agent/src-tauri/src/capture.rs:329](../../agent/src-tauri/src/capture.rs#L329). `capture.rs` deliberately (and correctly) merges "zero rows" and "all rows are previews" into one `EmptySession` outcome for `build_serato4` itself — that part is intentional and documented. `backfill.rs`'s sweep then treated `EmptySession` the same as "source gone / corrupt row" (generic `else { continue; }`), leaving the row frozen on stale pre-3.7 data with no signal distinguishing the two cases. **Arjun's ruling: log only, leave stale.** Added a new `CaptureError::AllPreviews` variant (distinct from `EmptySession`), returned only from the all-previews path; `backfill.rs` now matches it specifically and calls `reporter.report(...)` with the session identity before continuing — behavior (leave the row untouched) is unchanged, only the visibility differs.

**Patch** — applied 2026-08-04, full repo gate green after (agent fmt/clippy/350 tests; shared+web typecheck/lint/test; `supabase db reset` + 62 pgTAP)

- [x] [Review][Patch] `CapturedPlay.played_ms`/`library_added_at` lack `#[serde(default)]`, contradicting their own doc comment — [agent/src-tauri/src/store.rs:631](../../agent/src-tauri/src/store.rs#L631). The comment claims a pre-3.7 stored row "still round-trips," but serde errors on a genuinely *missing* JSON key for `Option<T>` fields unless `#[serde(default)]` is present (it only auto-`None`s a key present with an explicit `null`). The one real call site (`backfill.rs`'s `carry_forward_library_dates`) already treated a parse failure as a silent no-op, so the blast radius was narrow, but the comment was factually wrong and the pattern was fragile. Added `#[serde(default)]` to both fields and corrected the comment; new test `store::tests::captured_play_without_story_3_7_fields_round_trips_as_none` deserializes a genuine pre-3.7 shape (keys absent, not null).
- [x] [Review][Patch] `carry_forward_library_dates` keyed on `(started_at, title)`, which collides on duplicates (e.g. two stored plays both missing both fields) — [agent/src-tauri/src/backfill.rs:247](../../agent/src-tauri/src/backfill.rs#L247). A colliding key with two different stored dates silently resolved to whichever was seen last, defeating the "no data loss" guarantee the function claims. Fixed: the lookup map now tracks `Option<i64>` per key — a second, disagreeing date collapses the key to `None` (never guessed) instead of last-write-wins; a repeated matching date is unaffected. New tests: `carry_forward_skips_a_colliding_key_with_disagreeing_dates`, `carry_forward_applies_an_unambiguous_key`.
- [x] [Review][Patch] `resolve_played_ms`'s fallback bound searched past intervening plays with no `start_time` instead of stopping at the immediate next play — [agent/src-tauri/src/stats/mod.rs:216](../../agent/src-tauri/src/stats/mod.rs#L216). Silently folded an unresolvable intervening play's own gap into the current play's duration. Fixed: bound now comes from `plays.get(i + 1)` directly; a next play with no `start_time` leaves the duration unresolved rather than reaching further ahead. New test: `resolve_played_ms_does_not_search_past_a_next_play_with_no_start_time`.
- [x] [Review][Patch] No DB-level `CHECK` constraint on `plays.played_ms >= 0` despite the JSON schema declaring `minimum: 0` — [supabase/migrations/20260803190000_add_play_capture_fields.sql:21](../../supabase/migrations/20260803190000_add_play_capture_fields.sql#L21). Added `check (played_ms >= 0)`. New pgTAP case asserts a negative `played_ms` throws `23514` (plan bumped 13→14 in `sync_set_isolation_test.sql`).
- [x] [Review][Patch] `export_real_fixtures.rs`'s date-added coverage (`with_date`) was computed and logged but never asserted, unlike its `with_key`/`with_duration` siblings — [agent/src-tauri/tests/export_real_fixtures.rs:96](../../agent/src-tauri/tests/export_real_fixtures.rs#L96). Coverage is legitimately drive-dependent (25/105 USB-unplugged vs ~94% ceiling plugged in), so a fixed `>=90%` floor wasn't right, but a live regression collapsing to zero would have passed silently. Added a weak `with_date > 0` sanity floor.
- [x] [Review][Patch] `legacy.rs` never populates `total_length_ms` — [agent/src-tauri/src/joiner/legacy.rs:236](../../agent/src-tauri/src/joiner/legacy.rs#L236). AC-42 asks the legacy path to capture the same new fields "where present," sanity-check only (Arjun's library is serato4). **Resolved by disclosure, not implementation:** `database V2` does carry a `Field::Length(String)` (`len` tag), but its on-disk string convention (plain seconds? `M:SS`?) isn't verified against a real catalogue export the way `bpm`/`key`/`tadd`/`uadd` were in this diff's own verification pass — guessing the format would risk the fabricated-value failure mode AD-11 exists to prevent, for a field already scoped to sanity-check-only. Left absent, with the rationale now documented on `join()` instead of silently missing. Real fix needs someone to verify the format against an actual `database V2` file.

**Defer**

- [x] [Review][Defer] `DateAddedIndex::live` reimplements its own `/Volumes` directory scan instead of reusing the existing `watcher::detect::SystemDisks`/`DiskSource` abstraction. [agent/src-tauri/src/joiner/date_added.rs:88] — deferred: a real fix needs `DiskSource` extended beyond "install detection," an abstraction-design call, not a mechanical patch.
- [x] [Review][Defer] Catalogue loading in `DateAddedIndex::live` is synchronous, on whatever thread runs the watch loop, on the tick a session completes. [agent/src-tauri/src/watcher/mod.rs:488] — deferred: the code's own comment already scopes this to "only the rare tick that captures"; the same loop already does blocking SQLite I/O elsewhere, so this thread is very likely already off the main/UI thread, but this review couldn't confirm that from the diff alone.
- [x] [Review][Defer] No backfill-level test covers the played-flag filter's interaction with the pre-existing "re-derive already-captured serato4 sets" sweep. [agent/src-tauri/src/backfill.rs] — deferred: coverage nice-to-have, not blocking.

**Dismissed as noise (6)** — verified false positives or resolved by re-scoping:
- `DateAddedIndex`'s live lookup claimed to always return `None` (`strip_prefix` fails on a relative path) — independently raised by both Blind Hunter and Edge Case Hunter, but both misread `LegacyLibrary::get()`: it tries a direct-match lookup *before* the `strip_prefix` fallback, and `portable_id` is verified (this diff's own `serato-capture-completeness.md` update) as volume-root-relative, matching the catalogue's key format exactly. Not a bug.
- `library_added_at`'s wire type (`string`/ISO in the JSON schema) "contradicts" the SQL consumer casting it `::bigint` — a pre-existing, established codebase convention (`started_at` has the identical split since migration `20260731120000`, well before this diff); the new fields just consistently follow it.
- "No production code path converts `CapturedPlay` → `SyncPlay`" — `SyncPlay` is a TS-only type describing the *read* side (post-Postgres-conversion); the Rust agent forwards `plays_json` as a raw `serde_json::Value` to the RPC and never constructs a "SyncPlay" struct at all.
- `serato-capture-completeness.md` field-map not updated to "shipped" — false positive from this review's own backend-only diff scoping, which excluded `_bmad-output/`; the doc was in fact updated with real coverage numbers in the actual branch diff.
- `Field::DateAdded`/`Field::DateAddedStr` enum variants "not visible in the diff" — they're defined in the third-party `triseratops` crate, not project source; unreviewable from a diff alone, not evidence of a defect (the crate wouldn't compile against them otherwise).
- Test/fixture DDL-string duplication across `backfill.rs`/`capture.rs`/`watcher/mod.rs` — real but noise-level style nit, not worth a tracked finding.
