...
- This story remains genuinely blocked from live execution: no production domain is registered and no production Supabase project exists (both tracked in `pre-launch-services-checklist.md`, owned by other stories/decisions, not this one). Per the story's own completion bar (mirroring Story 2.1's `PROVISIONING.md` precedent), that live gap does not block marking this story done — only the runbook's accuracy and the two doc cross-references do, both of which are complete.
