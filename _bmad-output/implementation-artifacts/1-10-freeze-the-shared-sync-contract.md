...
1. **[PRODUCT — recommended default given] Should any per-play track-identity field (e.g. a hashed/opaque path token) be in the v1 frozen payload for Epic 4's future library-to-setlist correlation (FR-10)?** Task 2 recommends excluding the raw file path entirely from v1 (privacy: leaks local username/folder structure; no current UX consumer needs it). Additive-only means a purpose-built identity field can be added later without reopening this freeze, so the recommended default is to defer this decision to whichever Epic 4 story implements FR-10, rather than guess at its shape now. Proceed with exclusion unless Arjun prefers reserving a field now. **[RESOLVED 2026-08-07 by Story 4.2 (D-2/D-4):** the recommended default held — nothing was reserved at freeze time, and the field was added additively later, exactly as this question anticipated. `SyncPlay.track_id?: string | null` is `fnv1a_hex` of the track's volume-root-relative path (`agent/src-tauri/src/capture.rs::track_id`), reusing the hash already used for session identity. The raw path still never crosses the seam, so the privacy rationale for excluding it is intact — hashing is what made identity joinable without giving that up. A separate `SyncLibraryAddEvent` payload carries the same identity for tracks that were *added but never played*, which is the half FR-10 could not otherwise see at all. Recorded as proposed **AD-21** in `ARCHITECTURE-SPINE.md`.]
