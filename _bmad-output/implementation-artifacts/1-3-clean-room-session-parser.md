...
**Status: RF-1..RF-5 implemented and merged into the parser (see Dev Agent Record → Review-pass notes). Two deliberate deviations from the original tasks, both recorded below: a third `ParseError` variant (`Desync`) beyond Task 2's "exactly two", and start-time ordering in place of raw file order. Both deviations ratified by Arjun 2026-07-22 — no further action; Task 2's spec text is superseded by the two Deviation notes below.**
...
1. **[RESOLVED 2026-07-22] `master.sqlite` play-log scope gap** (Dev Notes above): Story 1.2's findings recommend prioritizing `master.sqlite` support since it's this DJ's actual live data source and needs no binary decoding, but no epics.md story owned reading play-log data directly from `master.sqlite`'s `history_session`/`history_entry` tables — Story 1.4 as written treats `master.sqlite` only as a metadata-join library. This story deliberately stayed scoped to the legacy `.session` file only.
   **Promotion rationale (elicitation pass, cascading-failure trace):** originally flagged as "decide before/during Story 1.4's creation." The trace moved it earlier. `~/Music/_Serato_/History/Sessions/` has been **frozen since 2025-12-11** — the watcher story was about to be pointed at a directory that no longer changes, and every downstream filter (joiner, stat-engine, local store, sync-queue) would be integration-tested only against a dead source. Epic 1 could finish fully green while producing **zero plays on the DJ's actual machine**.
   **Decision:** spun a new sibling story, **1.3b — `master.sqlite` play-log reader**, rather than folding into 1.4 — it produces the same `Play`-shaped ordered list this story does (a play-log *source*), which is a different concern from 1.4's metadata *enrichment* join, and conflating them would have widened 1.4's scope past its own ACs. Story 2.6 (folder auto-detection) and 2.8 (set capture) ACs were updated to select 1.3 vs. 1.3b per DJ install generation. See `epics.md` Story 1.3b and updated Story 2.6/2.8 ACs.
...
- **[Resolved 2026-07-22]** CI now lints test code: `.github/workflows/ci.yml`'s Clippy step runs `cargo clippy --manifest-path agent/src-tauri/Cargo.toml --all-targets -- -D warnings` (was missing `--all-targets`, so test-only code was invisible to the gate — how the two `redundant_closure` warnings above went undetected). Verified green locally against the current tree.
