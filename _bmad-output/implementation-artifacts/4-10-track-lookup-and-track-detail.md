...
- [x] [Review][Patch] New server-side, locale/timezone-dependent date rendering, ruled 2026-08-10 (Arjun): patch `TrackDetail`, accept-and-disclose `SetSimilarity`. — `TrackDetail.tsx`'s four `formatDayDate(...)` calls (library-added date, first/last played, per-set dates) rendered day labels server-side with no explicit timezone, the same defect class D-32 eliminated for the clock strip's hour. **Patched:** now render through a new `ClientDayDate` component (`web/app/components/track-detail/ClientDayDate.tsx`), the same `useSyncExternalStore` hydration-gate shape as `ClockStrip`, tested in `prop-threading.test.tsx`. **`libraryUtilization.ts`'s `buildAxisDayLabels()` (feeds `SetSimilarity`'s axis links) is architecturally different and was NOT patched**: its output also builds `setSimilaritySummary()`'s `aria-label`, which is a Server Component's accessible name and cannot be deferred to post-hydration without converting the whole grid to a Client Component — unlike `ClockStrip`, whose own `aria-label` is already client-owned, or `TrackSearch`, whose dates only ever render after a keystroke (guaranteed post-hydration). Accepted and logged in `deferred-work.md`, alongside the matching pre-existing entry for `GenreShareStream`/`TrendChart`'s locale-dependent axis ticks.
- [x] [Review][Patch] Search "no results" copy claims zero records exist even when a real match is hidden by the confidence reveal [`web/app/components/library-utilization/TrackSearch.tsx:167-180`] — `SearchResults` renders the blanket `NO_MATCH_COPY` whenever `visibleTrackSearchRows(matches, revealed)` is empty, including when `matches.length > 0` and the only reason nothing shows is the un-toggled reveal hiding a soundcheck-only match. Violates Non-negotiable 4 and reproduces the story's own named worst-offender (Story 4.7 R-2). **Fixed:** new `trackSearchNoMatchCopy(matchCount, visibleCount)` in `trackSearch.ts` distinguishes truly-no-match from hidden-by-reveal and states the hidden count, tested in `trackSearch.test.ts`.
- [x] [Review][Patch] `ClockStrip`'s accessible summary uses ranking language and has zero test coverage [`web/app/components/track-detail/ClockStrip.tsx` `clockSummary()`] — renders `"...more than any other"`, a superlative banned by Non-negotiable 6, inside a doc comment claiming "there is no 'top' or 'peak' language on screen." The only `ClockStrip` test asserts the pre-hydration placeholder; the hydrated bucket math and this string are never exercised. The three ranking-vocab regex sweeps in the test suite wouldn't catch this phrase either. **Fixed:** reworded to a count-of-total statement, `clockSummary` exported and unit-tested directly in a new `ClockStrip.test.ts`.
- [x] [Review][Patch] Duplicate-set axis links reintroduce `aria-hidden` on a still-focusable element [`web/app/components/library-utilization/SetSimilarity.tsx` `AxisLink`] — the `duplicate={true}` column-header case renders `<Link aria-hidden={true} tabIndex={-1} href=...>`; `tabIndex={-1}` removes it from the Tab sequence but the anchor is still focusable by other means, reintroducing the exact "focusable content inside an `aria-hidden` subtree" trap the same file's own doc comment describes fixing for the row axis one paragraph up. **Fixed:** duplicate column headers render as a non-interactive `<span>` instead (loses the mouse-click affordance on the column headers only; the row axis keeps it), `overflow-wrap` moved to the base `.lu-sim-axis` class so the wrap protection survives losing the `-link` class. Regression test added in `prop-threading.test.tsx`.
- [x] [Review][Patch] Corrupted/zero BPM renders as `"0"` instead of `"Unknown"` [`web/lib/sets/trackDetail.ts` `buildTrackIdentity`] — the guard is `Number.isFinite(play.bpm)`, and `0` passes it, contradicting D-8's "never a fabricated zero" applied everywhere else in this story. Currently unreachable on measured data (~100% coverage, no zero values seen). **Fixed:** guard changed to `play.bpm > 0`, tested in `trackDetail.test.ts`.

**Gate re-run after patches, in the `story/4-10-track-lookup-and-track-detail` worktree:** `pnpm lint` clean, `pnpm typecheck` clean, **732 tests / 32 files** passing (up from the story's own 596/28 baseline — 5 patches added targeted tests, no regressions), `pnpm build` exit 0.
- [x] [Review][Defer] Search can list the same physical track twice under a D-28 identity conflict [`web/lib/sets/trackSearch.ts` `buildTrackSearchIndex`] — deferred, pre-existing pattern: dedup only checks `playedIds`, so a `trackKey` group whose `track_id` fails closed to `null` (D-28) won't dedupe against a roster entry that still carries a valid id for the same track. Currently 0 instances in seed/prod per Task 1's own measurement; matches the "defensive only, cover with a unit test" posture the story already applies to D-28 elsewhere.
- [x] [Review][Defer] Per-module "no data" copy in `TrackDetail` can't distinguish "genuinely nothing" from "hidden by the confidence reveal" [`web/app/components/track-detail/TrackDetail.tsx` — ride time, mix neighbours, clock strip modules] — deferred, matches an existing pattern: the page-level `LibraryUtilizationReveal` banner already discloses the hidden count once, and this mirrors the exact shape Story 4.9's five modules already ship under the same wrapper. Not a new problem this story introduces.
- [x] [Review][Defer] D-36's literal placement clause ("inside the Tracks group") wasn't followed [`web/app/(authenticated)/library-utilization/page.tsx`, `LibraryUtilizationReveal.tsx`] — deferred, already disclosed: D-36's two clauses were mutually unsatisfiable, and the dev chose the one with a named failure mode, logging the deviation in both Completion Notes and `deferred-work.md` with the constraint preserved.
- [x] [Review][Defer] `.se-hidden-toggle` still fails the 24×24 target floor (63.5×19.5px) [shared `LowConfidenceReveal` component, also used on `/style-evolution`] — deferred, pre-existing: explicitly measured and flagged not fixed in this story's own Completion Notes (Task 9).
