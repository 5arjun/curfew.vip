...
- [x] [Review][Patch] **(resolved decision, fixed)** Auto-updater restarts the agent mid-active-capture with no guard — `updater_loop` (`lib.rs:263`) calls `app.restart()` immediately once `download_and_install` succeeds, with no check for an in-progress (not-yet-quiet-period) capture. **Arjun's ruling: gate it.** Fixed: new `store::has_active_capture` (any `Watching`-status row) + `lib.rs`'s `has_active_capture(&app)`, checked before `download_and_install` is even attempted — the tick is skipped entirely (retried next tick) rather than installing then deferring the restart.
- [x] [Review][Patch] **(resolved decision, fixed)** `has_format_drift` is a global, never-expiring signal that can permanently mask a real backlog — `sync_queue.rs:166/214` computes it as `store::has_unresolved_parse_failures(conn)`, true if *any* row exists in `parse_failures`, with no scoping to the current `WatchPlan`. `backfill.rs`'s own test `a_row_whose_source_is_no_longer_in_the_watch_plan_is_left_untouched` proves a row can become permanently unresolved (e.g. a DJ retires their legacy library), and since `has_format_drift` is checked *before* `has_permanent_backlog` in `desired_tray_state`'s precedence, an orphaned row would pin the tray on "format drift paused" forever and hide a genuine `Failed` state underneath it. **Arjun's ruling: scope it.** Fixed: new `store::has_unresolved_parse_failures_for_plan(conn, plan)` + `sync_queue.rs`'s `current_watch_plan(app)` (re-resolved fresh each pass, same cost class as `settings::validate_override`), wired into both `sync_loop`'s pass-level-`Err` branch and `handle_pass_outcome`.
- [x] [Review][Patch] **(resolved decision, fixed)** Backfill durability depends on external, agent-unowned state staying put — `parse_failures.raw_ref` is a pointer (`<db_path>#<session_id>` for Serato4, an absolute filesystem path for Legacy), not retained raw bytes, despite AC-2's wording ("backfill from raw data retained in local SQLite"). The Serato4 arm re-derives its DB location from the *current* `WatchPlan` on every reprocess attempt, but the Legacy arm reuses the exact original absolute path rather than re-resolving it against the current `plan.legacy.library_root` — so a drive remount/relabel breaks Legacy backfill even when "the same library" is present at a new root, an inconsistency the Serato4 path doesn't share. **Arjun's ruling: fix the Legacy/Serato4 inconsistency only.** Fixed: `backfill.rs`'s Legacy arm now rejoins the stored file name under the *current* `source.serato_dir.join("History").join("Sessions")` instead of trusting the stored absolute path. Full raw-byte retention and the "Serato prunes its own history" risk remain explicitly out of scope — logged to `deferred-work.md`.
- [x] [Review][Patch] **(fixed)** `updater_loop` silently swallows `check()`/`download_and_install()` errors [agent/src-tauri/src/lib.rs:260-266] — the `_ => {}` arm discards `Err(e)` with no log and no call into the `ErrorReporter` this same story built, so a broken `latest.json`, an expired signing key, or a network failure in the one mechanism this story's AC-2/AC-4 depend on to ever reach a DJ's machine is completely invisible. Fixed: both `check()` and `download_and_install()` error arms now call `error_reporting::SentryReporter.report(...)`.
- [x] [Review][Patch] **(fixed)** `unresolved_parse_failures` fails its entire batch on one bad row [agent/src-tauri/src/store.rs:557-561] — `.collect::<rusqlite::Result<Vec<_>>>()` returns `Err` on the first row with an unparseable `source`, and `backfill::reprocess_parse_failures`'s `let Ok(rows) = ... else { return; }` then silently skips the *whole* sweep for every session, not just the corrupt one. Fixed: now `filter_map`s per-row, skipping (and debug-logging) only the unreadable row instead of failing the whole read.
- [x] [Review][Patch] **(fixed)** `record_parse_failure`'s `ON CONFLICT` clause leaves `source`/`raw_ref` frozen [agent/src-tauri/src/store.rs:527-547] — only `failed_agent_version`/`failed_at`/`last_error` are in the `DO UPDATE SET` list, contradicting the function's own doc comment ("each failure overwrites the row with the latest attempt's info"). Fixed: `source`/`raw_ref` added to the `DO UPDATE SET` list.
- [x] [Review][Patch] **(fixed)** `SentryReporter::report` uses `configure_scope` instead of `with_scope` [agent/src-tauri/src/error_reporting.rs:37-41] — `configure_scope` permanently mutates the calling thread's persistent Hub scope rather than scoping tags to just this one capture. Since `watcher`/`sync_queue`/`backfill`/`updater_loop` each run on their own long-lived background thread, a later *unrelated* panic or report on the same thread inherits stale `agent_version`/`context` tags from whatever parse failure last reported there — undermining the exact "make drift attributable" goal this story exists to deliver, once Sentry is live. Fixed: swapped to `sentry::with_scope`.
- [x] [Review][Patch] **(fixed)** Unparseable `raw_ref` in the Serato4 backfill arm is silently dropped, never restamped [agent/src-tauri/src/backfill.rs:59-61] — if `parse_serato4_raw_ref` fails, the row is `continue`d before `capture_and_store_serato4` is ever called, so unlike every other continued-failure path, `record_parse_failure`/`reporter.report` never fire and `failed_agent_version` is never restamped. The row silently retries on *every* startup forever (not just after a version bump), invisible to Sentry. Fixed: that branch now calls `record_parse_failure` + `reporter.report` before continuing, same as every other continued-failure path.
- [x] [Review][Patch] **(fixed)** `clear_parse_failure`'s failure is silently discarded [agent/src-tauri/src/backfill.rs:85] — `.ok()` swallows the `Result` instead of routing through `log_store_err`, the convention every other store write in this diff follows. If the DELETE fails after a successful reprocess (DB locked/disk error), the ledger row survives despite the underlying session actually being recovered, permanently pinning the tray on `FormatDriftPaused`. Fixed: routed through `log_store_err` (bumped `pub(crate)`), matching every other store write in this file.
- [x] [Review][Patch] **(fixed)** Task 6(c)'s backfill test doesn't assert `last_error` was refreshed [agent/src-tauri/src/backfill.rs, `a_row_that_still_fails_stays_in_the_ledger_restamped_with_the_new_version`] — the task bullet asks the test to verify a still-failing row is "stamped with the new `agent_version` **and a fresh `last_error`**"; the test only asserts `failed_agent_version`. The underlying overwrite behavior is separately covered by a `store.rs` test, so this is a coverage-completeness gap, not a behavior gap. Fixed: added an `assert_ne!` on `last_error` against the original message.
- [x] [Review][Defer] Sentry "panic" feature + no event fingerprinting, both dormant until a real DSN is provisioned [agent/src-tauri/Cargo.toml sentry feature list; error_reporting.rs capture_message calls] — deferred, pre-existing/dormant: `capture_message` sends free-text `e.to_string()` with no `fingerprint`, which will likely fragment near-identical parse errors into many distinct Sentry issues instead of clustering under one alert; those same messages may embed local filesystem paths (potential OS-username PII) that `send_default_pii = false` does not redact; and the "panic" feature installs a global panic hook with no discussion of whether it changes this tray app's existing crash behavior. None of this is reachable today — `SentryReporter`/`init()` both no-op while `SENTRY_DSN` is empty — so it's deferred until Arjun actually provisions a DSN, at which point it's worth a fresh look.
