...
- **D-2 — Single centered column, whole-page scroll.** Flat "console rows" (label left, value/control right) separated by hairline rules — **not** the dashboard's card/glass vocabulary, which would read as decoration on a list of facts. Same whole-page-scroll break from the dashboard's viewport lock that 3.7 made. Max width ~720px inside the 1100px container. *(Amended 2026-08-05, Arjun — on-theme redesign, see story-file addendum: the flat-console MATERIAL clause is superseded — sections now sit in the app's `dz-shell` glass cards over the Silk backdrop, Hanken voice, abyss text ramp. The column/scroll/row-grammar clauses stand.)*
...
- **D-17 — Deliberately quiet motion.** No WebGL, no shader rim, no morphing numbers. Motion budget: the "Saved." fade, the confirm-dialog scrim, and standard focus/hover transitions. **The orphaned `LiquidMetalButton` does NOT get a demo home here** (the `deferred-work.md` entry floats Settings as a candidate) — a settings page is the wrong place to keep a hero material alive; it stays owned by the login/marketing stories. *(Amended 2026-08-05, Arjun — on-theme redesign, see story-file addendum: the blanket no-WebGL rule is lifted for the shared ambient `SilkBackdrop` only, and the motion budget now includes the redesign's card entrance/press transitions. The `LiquidMetalButton` decline stands — ambient backdrop ≠ hero-material showcase.)*
