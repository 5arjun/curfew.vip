...
- [x] [Review][Patch] `web/app/auth/confirm/route.ts` makes an unnecessary extra `supabase.auth.getUser()` call (lines 44-48) justified by a comment claiming "this route doesn't already have a user object in scope like callback/route.ts does" — false: both `exchangeCodeForSession()` and `verifyOtp()` already return `data.user` directly (confirmed against supabase-js's `AuthTokenResponse`/`AuthResponse` types), exactly like `callback/route.ts` already uses without a second call. This adds an avoidable network round-trip to GoTrue on every email confirmation, and if that extra call fails transiently, the shared try/catch flips `confirmed` back to `false`, sending an already-successfully-confirmed DJ to `/login?error=confirmation-failed` — a new false-failure mode this diff introduces for zero benefit. [web/app/auth/confirm/route.ts:32,35,44-48] **Fixed (2026-07-27):** both branches now capture `data.user` directly; the extra `getUser()` call is removed.
- [x] [Review][Patch] `web/app/phone-required/actions.ts`'s `setPhone` doesn't check the affected-row count on its `.update()` call — a zero-row update (e.g. a missing `djs` row) returns no PostgREST error, so `succeeded` is `true` and the DJ is redirected to `/` with nothing actually persisted; `needsPhone()`'s own fail-open error collapse would then treat that DJ as not needing a phone on their next visit too, so the gap self-perpetuates with zero error signal anywhere. Low reachability today (the `djs`-row-creation trigger is synchronous with the `auth.users` insert, so the row should always exist by the time this action runs), but the fix is cheap. [web/app/phone-required/actions.ts:36] **Fixed (2026-07-27):** `.update()` now chains `.select("id")`; `succeeded` requires a non-empty returned row set.
- [x] [Review][Patch] `web/app/phone-required/page.tsx`'s auth guard runs entirely client-side (`useEffect` + browser Supabase client) even though Task 3.2 explicitly allowed a server-side check, and both route handlers this story touches already gate server-side. Consequence: the page renders `null` (a blank flash) on every single visit while the client check resolves, and if JS fails to load/execute the page stays blank forever with no fallback — on a step EXPERIENCE.md calls "Not skippable." [web/app/phone-required/page.tsx:48-77] **Fixed (2026-07-27):** `page.tsx` is now a Server Component doing the `getUser()` check directly; the interactive form was split into a new client component, `phone-form.tsx`. Confirmed in the production build: `/phone-required` now lists as `ƒ` (server-rendered), not a static client page.
- [x] [Review][Patch] `web/app/phone-required/page.tsx`'s guard only checks "is authenticated," not "does this DJ still need a phone" — a DJ who already completed the flow can navigate back to `/phone-required` directly and silently overwrite their existing value with no warning or pre-fill. Self-only impact (RLS still scopes the update to the requester's own row), but an easy fix using the `needsPhone()` helper already imported elsewhere in this diff. [web/app/phone-required/page.tsx:54-73] **Fixed (2026-07-27):** folded into the same server-side guard above — `needsPhone()` is now checked server-side and redirects to `/` if the DJ already has a phone on file.
- [x] [Review][Patch] `PhoneActionState` is defined identically in both `actions.ts` and `page.tsx` instead of extracted to a shared module — the repo's own established precedent for this exact "use server" constraint (`web/app/login/auth-state.ts`, split out specifically so the type could be shared) was not followed; the Dev Agent Record's stated reason ("avoiding any ambiguity around what a Server Actions file may export") doesn't hold up against that existing precedent. Two structurally-identical definitions can now silently drift if either is edited alone. [web/app/phone-required/actions.ts:10-13, web/app/phone-required/page.tsx:12-15] **Fixed (2026-07-27):** extracted to a new `phone-state.ts`, mirroring `auth-state.ts`'s exact pattern; both `actions.ts` and `phone-form.tsx` now import from it.
- [x] [Review][Patch] `supabase/tests/djs_isolation_test.sql`'s Case 3e verification update (`update public.djs set phone = ... where id = DJ_B_ID`) is a bare statement, not wrapped in `lives_ok`/`throws_ok` — if a future regression made the column-scoped grant/RLS throw instead of silently filtering the row, this statement would raise an unhandled exception and abort the whole pgTAP file's transaction, rather than reporting a clean per-assertion failure among the file's 16 cases. [supabase/tests/djs_isolation_test.sql:114] **Fixed (2026-07-27):** wrapped in `select lives_ok(...)`; plan bumped 16 → 17. Re-ran `supabase test db supabase/tests`: `Files=1, Tests=17, Result: PASS`.
- [x] [Review][Defer] `web/app/login/actions.ts`'s `signIn()` always returns `"signed-in"` with no phone check [web/app/login/actions.ts:69] — deferred, this is the story's own documented "accepted residual limitation" for a returning DJ (Scope resolution section). Separately, `signUp()`'s `data.session === null ? "check-email" : "signed-in"` branch [web/app/login/actions.ts:49-53] would also skip the phone gate entirely if `enable_confirmations` were ever turned off in `supabase/config.toml` — dead under today's config, not an active bug, but the same class of gap and wasn't explicitly named in the story's Scope boundaries. Flagged for awareness; not actionable within this story per its own "do not add a third enforcement layer speculatively" ruling.
- [x] [Review][Defer] AD-19's column-scoped-grant requirement (this story's migration correctly satisfies it today) is enforced only by a comment in the migration file, not structurally [supabase/migrations/20260727192439_add_djs_phone_column.sql] — deferred, nothing in the schema or CI stops a future Epic 7 migration from re-introducing a blanket `grant update on public.djs`. Not this story's to fix; worth a schema-level guard (or a CI check alongside the existing additive-only migration check) whenever Epic 7's billing columns actually land.
...
- `web/app/phone-required/page.tsx`
- `web/app/phone-required/actions.ts`
- `web/app/phone-required/phone-form.tsx` (added during code review — split out of `page.tsx` when the auth guard moved server-side)
- `web/app/phone-required/phone-state.ts` (added during code review — shared `PhoneActionState` type, mirrors `auth-state.ts`)
