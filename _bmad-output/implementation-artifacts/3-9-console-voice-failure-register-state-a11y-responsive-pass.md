...
**Decision needed:** *(resolved by Arjun 2026-08-05 — see Patch list below)*

- [x] [Review][Decision] Heartbeat POST blocks the drain loop's own thread for up to 10s under network failure, delaying the next pass — contradicts Task 3's "never blocks, fails, or delays set sync" framing. **Resolved: shrink the beat-specific timeout** (option 2 of 3) — converted to a Patch item below. [agent/src-tauri/src/sync_queue.rs:197,217-231; agent/src-tauri/src/heartbeat.rs:94-113]
- [x] [Review][Decision] The six-state wire contract (Rust/SQL/TS) and the `STALE_AFTER_MS`/`MAX_INTERVAL` pairing are duplicated by hand across languages with no test catching a drift between them. **Resolved: add a lightweight cross-language contract check** (option 2 of 2) — converted to a Patch item below. [agent/src-tauri/src/tray.rs:96-105; supabase/migrations/20260805120000_create_agent_status.sql:103-105; web/lib/sets/agentStatus.ts:26-33,62-72]

**Patch:**

- [x] [Review][Patch] *(from Decision above)* Shrink the beat-specific `HTTP_TIMEOUT` in `heartbeat.rs` (currently 10s, shared with `sync.rs`'s general timeout) to a smaller value (e.g. 2-3s) so a network-down worst case delays the next drain pass by less, while staying fire-and-forget on the same thread. **Applied:** shrunk to 3s. [agent/src-tauri/src/heartbeat.rs:90-113]
- [x] [Review][Patch] *(from Decision above)* Add a lightweight cross-language contract check asserting the Rust `TrayState::wire_state()` set, the SQL `set_agent_status` allow-list, and TS `AGENT_SYNC_STATES` all agree, and that web `STALE_AFTER_MS` stays `2 × ` Rust `MAX_INTERVAL`. A Vitest test reading the Rust/SQL source files via `fs` and regex-extracting the literal lists (mirroring this repo's existing `no-hardcoded-colors.test.ts` static-analysis pattern) is the lightest-weight option that needs no new infra. **Applied:** `web/lib/sets/agentStatusContract.test.ts` (3 new tests). [agent/src-tauri/src/tray.rs:96-105; supabase/migrations/20260805120000_create_agent_status.sql:103-105; web/lib/sets/agentStatus.ts:26-33,62-72]

- [x] [Review][Patch] `Overlays.tsx` focus-restore can call `document.body.focus()` on close when the overlay was opened while `document.activeElement === document.body` — the common case on Safari, which does not focus a `<button>` on click (only on keyboard activation). This reproduces the exact "focus dropped to `<body>`" bug D-7 fixed, for ordinary mouse users on Safari. **Applied:** capture now excludes `document.body` as a valid restore target. [web/app/components/set-detail/Overlays.tsx:77-87]
- [x] [Review][Patch] `HeartbeatError::Rejected` (a `set_agent_status` 4xx — signals the wire-string contract itself has drifted, not that the agent is offline) is logged only under `#[cfg(debug_assertions)]`, identically to `Auth`/`Http` — so a real contract break is completely invisible in a release build. **Applied:** now logged unconditionally, mirroring `sync_loop`'s existing "permanent failure" branch; `Auth`/`Http` stay debug-only. [agent/src-tauri/src/sync_queue.rs:233-238]
- [x] [Review][Patch] `current_tray_state()`'s `.expect("current tray state mutex poisoned")` now sits on `sync_loop`'s hot path, called every drain pass forever — if the `CurrentTrayState` mutex is ever poisoned, the whole sync/heartbeat thread dies instead of just tray-icon drawing. **Applied:** recovers via `unwrap_or_else(|poisoned| poisoned.into_inner())` instead of panicking. [agent/src-tauri/src/tray.rs:272-280]
- [x] [Review][Patch] `AgentStatusBanner`'s `useState<AgentStatusSnapshot>(initial)` never re-syncs when the parent server component re-renders with a fresh `initial` prop (e.g. after a delete-set revalidation) — the banner shows a stale snapshot until the next 60s poll/focus event. **Applied:** reset during render (React's "adjusting state when a prop changes" pattern, tracking `prevInitial`), not via a `useEffect` — the effect+setState version tripped the same React Compiler lint this story's own D-5 already hit. [web/app/components/dashboard/AgentStatusBanner.tsx:22-33]
- [x] [Review][Patch] `role="status"` sits directly on the `<motion.p>` that `AnimatePresence key={line.text}` fully unmounts and remounts on every state change, rather than a persistent live region whose text mutates in place — a known ARIA-live pitfall where some assistive tech doesn't reliably announce a brand-new node appearing with `role="status"`. **Applied:** `role="status"` hoisted to a stable wrapper `<div>` outside the keyed swap. [web/app/components/dashboard/AgentStatusBanner.tsx:103-127]
- [x] [Review][Patch] `getAgentStatus()` collapses every Supabase error (missing env, broken RLS, a real misconfig) into the same silent `null` as the legitimate "no agent ever linked" case, with zero logging anywhere, even in dev — a real regression here would be indistinguishable from a healthy "nothing to report" and could sit invisible indefinitely. **Applied:** dev-only `console.error` added on both the Supabase-error and catch paths, mirroring the agent-side precedent. [web/lib/sets/index.ts:77-90]
- [x] ~~[Review][Patch] `SupabaseStatusClient::new()` (and its `reqwest::blocking::Client`) is rebuilt inside `beat_status` on every drain pass instead of being hoisted once, unlike the sibling `auth_client`/`sync_client` already hoisted in the same `sync_loop`.~~ **RETRACTED during implementation:** premise was wrong — `auth_client`/`sync_client` (sync_queue.rs:128-129) are declared *inside* `loop { ... }`, not hoisted above it, so they're rebuilt fresh every iteration too. `beat_status`'s fresh-client-per-pass is consistent with this function's existing convention, not an outlier; hoisting only the new one would make the loop *more* inconsistent, and touching the pre-existing two is outside this diff's scope. No code change. [agent/src-tauri/src/sync_queue.rs:125-131,217-231]
- [x] [Review][Patch] The AC-4 Completion Notes attestation ("7 hits, all confirmed false positives") does not reproduce: re-running the cited grep now returns **9** hits, and the note's own itemized breakdown (3 comments + `auth-biometric-badge` ×2 + `streaks` ×1 = 6) doesn't even sum to the 7 it claims. All 9 hits genuinely are false positives (independently verified — the invariant holds), including 2 uncounted hits inside this diff's own new `status-copy.test.ts`. **Applied:** Completion Notes corrected to the actual 9-hit breakdown. [Completion Notes, AC-4]
- [x] [Review][Patch] `set_agent_status(sync_state text)` has no default; an authenticated caller (any DJ, not just the agent) POSTing without the `sync_state` key gets an opaque PostgREST "function not found" error instead of the function's own controlled validation message. **Applied:** `default null` added; verified against the pgTAP suite post-`db reset`. [supabase/migrations/20260805120000_create_agent_status.sql:75]
- [x] [Review][Patch] "`STALE_AFTER = 600s` ... with margin" is stated in three places (SQL comment, `agentStatus.ts` doc comment, story Dev Notes) but is exactly `2 × MAX_INTERVAL` with zero slack for scheduling jitter or a slow POST. **Applied:** the SQL comment never actually said "with margin" (finding overstated that); fixed `agentStatus.ts`'s doc comment and its test description/assertion. Story Dev Notes text (lines 27, 73) left untouched — it's Arjun's original ruling quote, not an engineering claim to correct. [web/lib/sets/agentStatus.ts:62-72; web/lib/sets/agentStatus.test.ts:12-19]

**Defer:**

- [x] [Review][Defer] `set_agent_status`'s upsert surfaces a raw Postgres FK-violation error rather than the function's own 42501/22023 contract if the caller's `djs` row is deleted while their JWT is still valid — mirrors an already-accepted gap in the sibling `sync_set` RPC this migration deliberately copies; the fire-and-forget agent path treats it identically to any other rejected beat regardless. [supabase/migrations/20260805120000_create_agent_status.sql:110-114] — deferred, pre-existing pattern

**Dismissed as noise (4):** "cross-fade" implemented as a sequential fade (`mode="wait"`) rather than a literal overlap — well-reasoned in-code tradeoff (avoids layout push) that still satisfies the actual "never snap/slide" constraint; silence overloaded across 5 situations (no-agent vs. idle vs. stale vs. error all render nothing) — covered by the settled "silence for Idle/DriveNotConnected is intentional" ruling, onboarding-nudge design is out of this story's scope; AC-5's manually pixel-sampled contrast numbers aren't checked in as a reproducible artifact — a documentation-rigor note, not a code defect; the dead `try_state` fallback in `beat_status` (unreachable given the unconditional `.state()` call earlier in the same loop iteration) — real but purely cosmetic, no behavior risk.
...
**AC-4 — no-celebration invariant. Verified and attested.** Grep over `web/app`, `web/lib`, `agent/src-tauri/src`, `shared/src` for `streak|confetti|badge|milestone|crushing|awesome|great job|nice work|congrats|well done|keep it up|🔥|🎉|🎊|🏆|⭐|celebrat` returns **9 hits, all confirmed false positives by reading each one** *(corrected during code review, 2026-08-05 — the original write-up here undercounted at 7 and its own itemized breakdown only summed to 6; re-run against the final tree below)*: `dashboard.css` (a comment: "no celebration"), `globals.css`/`BiometricAnchor.tsx` ×3 (`auth-biometric-badge`, the fingerprint pill on the login screen — a UI affordance, not an achievement), `status-copy.ts` (a comment asserting the invariant), `status-copy.test.ts` ×2 (this test's own description and its banned-word regex literal — the two hits missing from the original count), `page.tsx` (a comment: "no celebration, no alarm"), `FloatingNav.tsx` (`streaks` describing the LiquidMetal shader's specular highlights). Zero genuine hits. The new status region adds no celebratory affordance — it has no icon, no count, no colour, and its *healthy* state is silence (a working sync is deliberately not news). The `StatsColumn` enrichment placeholder remains `aria-hidden` and gates no current stat.
...
- **2026-08-05** — Code review pass (see Review Findings above). 12 patches applied: Safari focus-restore regression fix (`Overlays.tsx`, the review's highest-severity finding — mouse clicks on Safari don't focus `<button>`, so the D-7 fix could restore to `document.body`), release-build heartbeat-rejection logging, poison-safe tray mutex read, `AgentStatusBanner` prop re-sync + `role="status"` hoisted to a stable wrapper, dev-only `getAgentStatus()` error logging, beat HTTP timeout shrunk 10s→3s, new cross-language wire-contract test, AC-4 grep count corrected (7→9, all still false positives), `set_agent_status` RPC default param, `STALE_AFTER_MS` "with margin" wording fixed. 1 item deferred to `deferred-work.md` (FK-violation raw-error edge, mirrors accepted `sync_set` gap). 1 client-hoisting patch retracted mid-implementation (false premise — sibling clients aren't hoisted either).
